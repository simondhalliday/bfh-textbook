...
#text(x = c(0, 4, 12, xlims[2]), par("usr")[3] - 0.4, labels = xlabels, srt = 0, pos = 1, xpd = TRUE)
...
#lines(xx1, AvgRevenue(xx1, rmax = 12, xmax = 12), col = COLA[4], lwd = graphlinewidth)
...
text(-1.2, 0.5*ylims[2], expression(paste("Price, ", p, " ($)")), xpd = TRUE, cex = axislabelsize, srt = 90) 

#Label curves
text(xlims[2] - 1, 7, expression(paste("Iso-Profit \n Curves")), cex = labelsize)
...
text(xlims[2] - 2.5, 3.5, expression(paste("Average cost")), cex = labelsize)
