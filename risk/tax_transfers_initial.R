...
require(shape)
library(pBrackets)
pdf(file = "risk/tax_transfers_initial.pdf", width = 9, height = 7)
...
ticksy <- c(0, 3, 5.5, 9.55, 12.2, ylims[2])
ylabels <- c(NA, expression(paste(underline(y)(1 - phi))), expression(hat(y)[f*minute]), expression(hat(y)[f]), expression(hat(y)[j]), NA)
ticksx <- c(0, 3.33, 8.74, xlims[2])
xlabels <- c(NA, expression(Delta[f*minute]), expression(Delta[f]), NA)
...
#Segment to j
segments(0, 12.2, 8.74, 12.2, col = grays[20], lwd = segmentlinewidth, lty = 2)
...
text(10.5, 8.4, expression(paste("Reduced")), xpd = TRUE, cex = labelsize)
text(10.5, 7.75, expression(paste("expected")), xpd = TRUE, cex = labelsize)
text(10.5, 7.3, expression(paste("income")), xpd = TRUE, cex = labelsize)
text(10.5, 6.8, expression(paste("with taxes")), xpd = TRUE, cex = labelsize)
