...
library(shape)
pdf(file = "credit/credit_barriers_iso.pdf", width = 8, height = 6)
...
isoprofitFn <- function(delta, rho = 0.1, b = 0.5) {
  1 - ((1 + rho)/(1 - b))*(1/delta)
}
...
ylabels <- c(NA, 1)
ticksx <- c(xlims[1], (1 + 0.1)/(1 - 0.5), xlims[2])
xlabels <- c(NA, NA, NA)
...
lines(xx1, isoprofitFn(xx1), col = COLB[4], lwd = graphlinewidth)
...
text(9, 0.68, expression(paste(hat(pi)[1]({b>0}) )), cex = labelsize, xpd = TRUE)
...
text(13, 0.26, expression(paste("Lenders")), cex = labelsize)
text(13, 0.2, expression(paste("entering")), cex = labelsize)
