...
library(shape)
pdf(file = "credit/non_contractual_brf_indiffs.pdf", width = 8, height = 6)
...
ylabels <- c(NA, expression(paste(f == frac(1,2))), expression(paste(f*(delta^b) )),  expression(paste(f*(delta^a) == frac(3,4))),expression(paste(f*(delta^c) )), NA)
ticksx <- c(xlims[1], 0.34, 0.5, 0.65, xlims[2])
xlabels <- c(NA,  expression(paste(delta^b)), expression(paste(delta^a)), expression(paste(delta^c)), NA)
...
segments(0.5, ylow(delta = 0.5), 0.5, yhigh(delta = 0.5), lty = 2, col = "gray" , lwd = segmentlinewidth)
...
text(0.5+ 0.02, brfFn(delta = 0.5) - 0.02, expression(paste(a)), cex = labelsize)
...
text(0.2, 1.01, expression(paste(y[1])), cex = labelsize)
