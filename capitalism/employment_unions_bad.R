require(shape)
require(plotrix)
pdf(file = "capitalism/employment_unions_bad.pdf", width = 9, height = 7)
...
text(0.99, 39, expression(paste("Wage curve")), cex = labelsize,xpd = TRUE)
text(0.99, 37, expression(paste(w[0]^N*(H))), cex = labelsize,xpd = TRUE)

text(0.64, 39, expression(paste(w[1]^N*(H))), cex = labelsize, xpd = TRUE)

#segments(1, 0, 1, 42, lty = 2, lwd = 3, col = "darkgray")
segments(0.75, 0, 0.75, 20, lty = 2, lwd = segmentlinewidth, col = grays[20])
...
segments(0, 20, 0.75, 20, lty = 1, lwd = segmentlinewidth, col = COLB[4])
...
lines(xx1, WageFn(xx1, delta = 10), col = COLA[4], lwd = graphlinewidth)
...
Arrows(0.75, 22, 0.75, 36, col = "black", lty = 1, lwd = 2, arr.type = "triangle")
...
segments(0.5, 0, 0.5, 20, lty = 2, lwd = segmentlinewidth, col = grays[20])
