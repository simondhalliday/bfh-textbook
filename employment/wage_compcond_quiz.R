...
require(plotrix)
require(pBrackets)
#pdf(file = "employment/wage_compcond_quizc.pdf", width = 9, height = 7)
jpeg(file = "employment/wage_compcond_quiz.jpeg", width = 6*120, height = 4.5*120)
...
nsWage <- function(h, ubar = 0.05, B = 0.2, t = 0.8) {
  B + ubar + (ubar - ubar*t +  ubar*t*h )/(t*(1 - h))
}
...
text(-0.11, 20, expression(paste("Wage, ", w)), cex = axislabelsize, xpd = TRUE, srt = 90)
...
text(0.92, 43, expression(paste("Wage curve")), cex = labelsize, xpd = TRUE)
text(0.92, 41, expression(paste(w^N*(H))), cex = labelsize, xpd = TRUE)
...
segments(0.7917, 0, 0.7917, 20, lty = 2, lwd = segmentlinewidth, col = grays[20])
