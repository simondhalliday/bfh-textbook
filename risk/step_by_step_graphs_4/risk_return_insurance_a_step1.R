...
require(shape)
library(pBrackets)
pdf(file = "risk/step_by_step_graphs_4/risk_return_insurance_a_step1.pdf", width = 9, height = 7)
...
ticksy <- c(0,  riskreturn(g = 5.0), insurance(g = 1.5), insurance(g = 10.5), ylims[2])
ylabels <- c(NA, expression(paste(omega[i]^b)), expression(paste(omega^n)), NA, NA)
