...
library(shape)
pdf(file = "mathappendix/logarithm.pdf", width = 8, height = 6)
...
xlabels <- c(NA, 5, expression(x))
ticksy <- c(ylims[1], 0, 2, ylims[2])
ylabels <- c(-1, 0, 2, expression(y))
...
# lines
lines(xx1, log(xx1), col = COLB[4], lwd = graphlinewidth)
