...
library(shape)
require(shape)
pdf(file = "/Users/rileyboeth/Library/Mobile Documents/com~apple~CloudDocs/bfh textbook - backup/coordination_failures/figure4_3.pdf", width = 10, height = 8)
...
segments(2, ConcaveU(2), 36, ConcaveU(36), lty = 1, col = COLA[6] , lwd = graphlinewidth)
...
text(13, -.9, expression(paste("y = y"[ce])),  xpd = TRUE,  cex = labelsize)
...
text(-2.8, ConcaveU(13), expression(paste("Pv(y- ",Delta,"y) + ")),  xpd = TRUE, cex = labelsize)
text(-4.2, ConcaveU(13)-1.5, expression(paste("(1 - P)(v)(y + ",Delta,"y)")),  xpd = TRUE, cex = labelsize)
text(-1.8, ConcaveU(13)-3, expression(paste(" = v(y)")),  xpd = TRUE, cex = labelsize)
text(-2.7, ConcaveU(2), expression(paste("v(y - ",Delta,"y)")),  xpd = TRUE,  cex = labelsize)
text(-1.5, ConcaveU(23.18), expression(paste("v(y)")),  xpd = TRUE,  cex = labelsize)

#Add risk premium distance arrow and label

Arrows(13.6, ConcaveU(2), 22.58, ConcaveU(2), col = "black", lty = 1, lwd = 2, code = 3, arr.type = "triangle", arr.lwd = 0.5)
text((23.18+13)/2, ConcaveU(2)+1, expression(paste("Risk Premium")),  xpd = TRUE,  cex = labelsize)
