require(shape)
pdf(file = "firmmarketsupply/supply_stepwise_firmj.pdf", width = 9, height = 7)
...
xlabels <- c(NA, expression(paste(underline(x)^J)), NA)
...
#Line for Supply
##Firm J
segments(shares[1], costs[2], shares[2], costs[2], lty = 1, col = COLB[4] , lwd = segmentlinewidth)
segments(shares[2], costs[2], shares[2], ylims[2], lty = 1, col = COLB[4] , lwd = segmentlinewidth)
...
text(0.5*xlims[2], -0.3, expression(paste("Firm J's output, ", x^J)), xpd = TRUE, cex = axislabelsize) 
...
text(2.3, 2.8, expression("Firm J's"), cex = labelsize)
...
#Indicator lines
#Vertical lines
segments(shares[2], 0, shares[2], costs[2], lty = 2, col = "darkgrey", lwd = segmentlinewidth)
...
segments(0, costs[3], xlims[2], costs[3], lty = 2, col = "darkgrey", lwd = segmentlinewidth)
