...
library(shape)
pdf(file = "public_mechanism/second_best_baseline.pdf", width = 9, height = 7)
...
xpolya <- c(monopoly[1], monopoly[1], 0, 0, monopoly[1])
...
text(1, 8.25, expression(paste(CS^{MP})), cex = labelsize)
text(1, 7, expression(paste(pi[msc]^{MP})), cex = labelsize)
text(1, 4.75, expression(paste(pi[mpc]^{MP})), cex = labelsize)
...
text(1.3, 2.25, expression(paste("Monopoly")), cex = labelsize)
text(1.3, 1.75, expression(paste("only incurs")), cex = labelsize)
text(1.3, 1.25, expression(paste("private costs")), cex = labelsize)
