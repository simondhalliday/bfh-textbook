...
ticksy <- c(0,  riskreturn(g = 5.6), insurance(g = 1.8) ,ylims[2])
ylabels <- c(NA, expression(paste(omega^u)), expression(paste(omega^{2*i})), NA)
ticksx <- c(0, 1.8, 5.6, 10.5, xlims[2])
xlabels <- c(NA, expression(paste(Delta^{2*i})), expression(paste(Delta^u)), expression(paste(Delta^{1*i})), NA)
...
text(xlims[1] - 1.5, ylims[2] - 0.5*(ylims[2] - ylims[1]), expression(paste("Expected income, ", omega)), xpd = TRUE, cex = axislabelsize, srt = 90) 
...
text(5.6 + 0.25, riskreturn(g = 5.6) - 0.3, expression(u), cex = labelsize)
...
text(11.5, 12.1, expression(paste(p%.%i)), xpd = TRUE, cex = labelsize)
...
text(9.1, 17, expression(u[1]), cex = labelsize)
text(7.5, 17, expression(u[2]), cex = labelsize)
