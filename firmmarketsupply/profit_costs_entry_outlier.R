...
png(file = "firmmarketsupply/profit_costs_entry_outlier.png", width = 7*120, height = 5*120)
...
ticksy <- c(0, costs[1], AvgCost(x = entQ[1]),  ylims[2])
ylabels <- c(NA, expression(paste(mc == c)), expression(paste(ac, "(", q[m],")")), NA)
ticksx <- c(0, entQ[1], xlims[2])
...
text(-1.6, 0.5*ylims[2], expression(paste("Price per unit, revenue, and costs ($) ")), xpd = TRUE, cex = axislabelsize, srt = 90) 
...
Arrows(6.9, 7.4, entQ[1] + 0.15, MRevenue(entQ[1]) + 0.2, col = "black", lty = 1, lwd = 2, arr.type = "triangle", arr.lwd = 0.5)
