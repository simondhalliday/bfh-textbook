require(shape)
pdf(file = "employment/employment_price_initial_indiff_ed.pdf", width = 9, height = 7)
...
npts <- 501 
e <- seq(ylims[1], 0.99, length.out = npts)
y <- seq(xlims[1], xlims[2], length.out = npts) 
a <- c(-3, -1.85, 3)
...
# lines(xx5, indiffFn1(xx5, u = 0), col = COLB[4], lwd = graphlinewidth)
# lines(xx5, indiffFn1(xx5, u = -1.5), col = COLB[4], lwd = graphlinewidth)
# lines(xx3, indiffFn1(xx3, u = 1.5), col = COLB[4], lwd = graphlinewidth)
...
text(8, 0.5, expression(paste(mrs(p,e) == frac(u[p],u[e]), phantom() == -frac( (1 - e)^2, delta ) )), cex = labelsize)
