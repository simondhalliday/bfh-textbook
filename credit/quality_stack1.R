...
ylabels <- c(NA, expression(paste(f == frac(1,2))), expression(paste(f == frac(3,4))), NA)
...
text(-0.22, 0.5*(ylims[2]), expression(paste("Probability of failure (risk), ", f)), xpd = TRUE, cex = axislabelsize, srt = 90) 

segments(0, 0.75, 1, 0.75, lty = 2, col = "gray" , lwd = segmentlinewidth)
# segments(1.5, 0, 1.5, brfFn(delta = 1.5, k = 0.666), lty = 2, col = "gray" , lwd = segmentlinewidth)

# segments(1.55, 0, 1.55, brfFn(delta = 1.55, k = 0.5), lty = 2, col = "gray" , lwd = segmentlinewidth)
# points(1.55, brfFn(1.55, k = 0.5), pch = 16, col = "black", cex = 1.5)

segments(0.46, -1, 0.46, brfFn(0.46, k = 0.5), lty = 2, col = "gray" , lwd = segmentlinewidth)
points(0.46, brfFn(0.46, k = 0.5), pch = 16, col = "black", cex = 1.5)
...
# text(1.5 + 0.04, brfFn(1.5, k = 0.75) - 0.03, expression(paste(b)), cex = labelsize)
...
text(0.1, 0.43, expression(paste(1 + rho[t+1])), cex = labelsize)
Arrows(0.17, 0.43, 0.26, 0.43, col = "black", lty = 1, lwd = 2, arr.type = "triangle", arr.lwd = 0.5)
text(0.1, 0.25, expression(paste(1 + rho[t])), cex = labelsize)
