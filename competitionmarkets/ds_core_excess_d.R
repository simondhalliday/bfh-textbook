require(shape)
pdf(file = "competitionmarkets/ds_core_excess_d.pdf", width = 9, height = 7)
...
ylabels <- c(NA, expression(paste(p[L])), expression(paste(p,"*")), expression(paste(p[f])), NA, NA)
ticksx <- c(0,  60, 75, 84, 120, xlims[2])
xlabels <- c(NA, expression(paste(X[{SL}])), expression(paste(X,"*")), expression(paste(X[{DL}])), NA, NA)
