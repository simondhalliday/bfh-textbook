...
require(shape)
library(pBrackets)
pdf(file = "capitalism/insurance_marginfigure.pdf", width = 7, height = 5)
...
ylabels <- c(NA, NA, expression(paste(hat(y)[d])), expression(paste(hat(y)[c])), NA)
...
text(xlims[2] - 0.5*(xlims[2] - xlims[1]), ylims[1] - 2.8, expression(paste("Degree of risk, ", Delta)), xpd = TRUE, cex = axislabelsize) 
text(xlims[1] - 2, ylims[2] - 0.5*(ylims[2] - ylims[1]), expression(paste("Expected income, ", hat(y))), xpd = TRUE, cex = axislabelsize, srt = 90) 

text(-0.8, 8.5, expression(paste(hat(y)[a])), xpd = TRUE, cex = labelsize) 
...
text(5.6 + 0.4, riskreturn(g = 5.6) - 0.4, expression(a), cex = labelsize)
...
text(1.8, insurance(g = 1.8) + 1, expression(paste(d)), cex = labelsize)
...
text(10.5, riskreturn(g = 10.5) + 0.8, expression(paste(c)), cex = labelsize)
...
text(14.5, 12, expression(paste(hat(y)(Delta))), cex = labelsize, xpd = TRUE)


#Label value functions
text(10.5, 18.6, expression(u[1]), cex = labelsize, xpd = TRUE)
text(9.5, 18.6, expression(u[2]), cex = labelsize, xpd = TRUE)
text(8.5, 18.6, expression(u[3]), cex = labelsize, xpd = TRUE)
