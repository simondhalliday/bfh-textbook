require(shape)
pdf(file = "employment/employment_brf_nash_employment_sub.pdf", width = 9, height = 7)
...
solowCondition2 <- function(w, delta = 2){
  ((w-7)*(1/(9*(delta))))
}

solowCondition3 <- function(w, delta = 5){
  (w*(1/(8*delta)))
}
...
xx9 <- seq(0, xlims[2], length.out = npts2)
yy1 <- isovhigh(xx7, delta = 5, v = 20)
yy2 <- isovhigh3(xx12, v = 12, delta = 5)
xx10 <- seq(xlims[1], 20, length.out = npts)
xx11 <- seq(xlims[1] + 0.5, xlims[2], length.out = npts2)
xx12 <- seq(14, xlims[2], length.out = npts2)
xx13 <- seq(xlims[1], 25, length.out = npts2)


#polygon(c(xx7, xx7[340]), c(yy1, yy1[340]), col = COL[9], density = NULL, border = NA)

#polygon(c(xx12, xx12[225]), c(yy2, yy2[225]), col = "#FFC300", density = NULL, border = NA)
#Draw the lines for the graphs
lines(xx1, brfFn(xx1), col = "#beaed4", lwd = graphlinewidth)
lines(xx0, isov(xx0, delta = 5), col = COL[6], lwd = graphlinewidth)

lines(xx9, solowCondition2(xx9, delta = 2.64), col = c("#E10D07"), lwd = graphlinewidth)
lines(xx11, solowCondition3(xx11, delta = 5), col = COLB[4], lwd = graphlinewidth)
...
#lines(xx5, isovhigh1(xx5, v = 10, delta = 5), col = COL[6], lwd = graphlinewidth, lty = 2)
#lines(xx6, isovlow1(xx6, v = 20, delta = 5), col = COL[6], lwd = graphlinewidth, lty = 2)

lines(xx12, isovhigh3(xx12, v = 11.7, delta = 5), col = COL[6], lwd = graphlinewidth)
lines(xx13, isovlow3(xx13, v = 11.7, delta = 5), col = COL[6], lwd = graphlinewidth)

#Customize ticks and labels for the plot
ticksy <- c(0, 0.35, 0.5, 1)
#ylabels <- c(0, expression(paste(frac(1,2))), 1)
ylabels <- c(0,  expression(paste(e^N[s])), expression(paste(e^N)), 1 )
ticksx <- c(0, NA, 7, 15.3, 20, 40)
xlabels <- c(0, NA ,expression(paste(s)), expression(paste(w^N[s])),  expression(paste(w^N)) , NA)
axis(1, at = ticksx, pos = 0, labels = xlabels, cex.axis = labelsize)
axis(2, at = ticksy, pos = 0, labels = ylabels, las = 1, cex.axis = labelsize)

#Annotation of  the three graphs and the NE
text(42, 0.96, expression(paste("Slope: ", frac(e, w))), cex = labelsize, xpd = TRUE)
text(20, 0.96, expression(paste("Slope: ", frac(e, w-s))), cex = labelsize, xpd = TRUE)
#text(25, 0.92, expression(paste(frac(e, w))), cex = labelsize)
Arrows(25, 0.95, 28, 0.95,  col = "black", lty = 1, lwd = 2, arr.type = "triangle")
...
text(34, 0.65, expression(paste("BRF (ICC)")), cex = labelsize, xpd = TRUE)
...
text(37, 0.98, expression(paste(c[1])), cex = annotatesize)
text(29, 0.98, expression(paste(c[2])), cex = annotatesize)
#text(18, 0.98, expression(paste(c[1])), cex = annotatesize)
#text(4, 0.38, expression("Participation"), cex = labelsize-0.05)
text(4.7, 0.31, expression("PC:"), cex = labelsize-0.05)
text(4.7, 0.25, expression(paste(u[0],phantom())), cex = labelsize)
...
segments(15.3, 0, 15.3, 0.35, lty = 2, col = grays[20], lwd = segmentlinewidth)
segments(0, 0.35, 15.3, 0.35, lty = 2, col = grays[20], lwd = segmentlinewidth)
...
text(20.9, 0.47, expression(n), cex = labelsize)
text(16.3, 0.33, expression(n[s]), cex = labelsize)
text(8.6, 0.33, expression(c[s]), cex = labelsize)
points(7.7, 0.35, pch = 16, col = "black", cex = 1.5)
...
points(15.3, 0.35, pch = 16, col = "black", cex = 1.5)
...
#spoints(22, isovlow3(22, v = 20, delta = 5), pch = 16, col = "black", cex = 1.5)
