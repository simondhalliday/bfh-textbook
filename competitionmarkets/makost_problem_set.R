require(shape)
pdf(file = "competitionmarkets/makost_problem_set.pdf", width = 9, height = 7)
...
ticksy <- c(0, 1, 2, 4, 5, ylims[2])
ylabels <- c(NA,  expression(paste(c[1]^F)),  expression(paste(c[1]^G)), expression(paste(r[max]^B)), expression(paste(r[max]^A)), NA)
...
segments(1, 4, 2.04, 4, lty = 1, col = COLA[4] , lwd = segmentlinewidth)
segments(2.04, 4, 2.04, 0, lty = 1, col = COLA[4] , lwd = segmentlinewidth)
...
segments(2, 2, 2, ylims[2], lty = 1, col = COLB[4] , lwd = segmentlinewidth)
...
segments(0, 4, 1, 4, lty = 2, "gray" , lwd = segmentlinewidth)
...
text(2.2, 0.7, expression("Buyers'"), cex = labelsize)
text(2.2, 0.45, expression("Demand"), cex = labelsize)
text(2.2, 5.7, expression("Sellers'"), cex = labelsize)
text(2.2, 5.45, expression("Supply"), cex = labelsize)
...
text(0.32, 4.6, expression("Equilibrium price"), cex = labelsize)
text(0.32, 4.35, expression("falls in this range"), cex = labelsize)
Arrows(0.65, 4.1, 0.65, 4.9, col = "black", code =3, lty = 1, lwd = 2, arr.type = "triangle", arr.lwd = 0.5)
