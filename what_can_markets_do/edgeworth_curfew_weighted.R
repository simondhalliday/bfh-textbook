...
uA <- function(x, y, alpha = 1/2, Ta = 1) {
  y - alpha*(Ta - x)^2
}

indiffA <- function(x, utility = -18, alpha = 1/2, Ta = 1) {
  utility + alpha*(Ta - x)^2
}

uB <- function(x, y, beta = 1/2, Tb = 7) {
  -(y) - beta*(Tb - x)^2
}

indiffB <- function(x, utility = 0, beta = 1/2, Tb = 7) {
  utility - beta*(x - Tb)^2
}
...
ticksy <- seq(from = -24, to = 24, by = 2)
ylabels <- seq(from = -24, to = 24, by = 2)
