require(shape)
require(plotrix)
pdf(file = "capitalism/macro_oppcost_capital.pdf", width = 9, height = 7)
...
ylabels <- c(NA, expression(paste(w[0]^c)), expression(paste(w[1]^c)), expression(paste(gamma[0])), NA )
...
Arrows(0.1, 20.5, 0.1, 28.5, col = "black", lty = 1, lwd = 2, arr.type = "triangle")
...
text(0.866 + 0.02, 30 - 1.5, expression(paste(n[1])), cex = labelsize)


text(0.2, 35.7, expression(paste("New competition")), cex = labelsize, xpd = TRUE)
text(0.2, 33.7, expression(paste("condition ")), cex = labelsize, xpd = TRUE)
text(0.2, 31.5, expression(paste(w^c == w[1]^c)), cex = labelsize, xpd = TRUE)
...
text(0.2, 13.8, expression(paste(w^c == w[0]^c)), cex = labelsize, xpd = TRUE)
