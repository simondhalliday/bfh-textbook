#require(ggplot2)
require(shape)
pdf(file = "what_can_markets_do/edgeworth_walras_newz.pdf", width = 9, height = 7)
...
CheckPriceHigh <- function(x) {
  33.5 - ((7 + 2/9)/2)*x
}

CheckPriceLow <- function(x) {
  3.5 - (1/((7 + 2/9)/2))*x
}

#Deriving the offer curves: let py = 1
#wA = 8px + 2py = 8px + 2; 
#Demand: x = w/2px => x = 4 + 1/px => px = 1/(x - 4)
...
lines(xx1, CheckPriceHigh(xx1), col = COL[8], lwd = graphlinewidth)
lines(xx1, CheckPriceLow(xx1), col = COL[8], lwd = graphlinewidth)
...
text(6.4, 8, expression(paste("Price ", phantom() == 3.61)))
...
points(4.83, (3/2)*4.83, pch = 16, col = "black", cex = 1.5)
text(4.83 - 0.1, 6.7, expression(w))
