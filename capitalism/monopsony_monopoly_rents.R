...
library(plotrix)
library(pBrackets)
pdf(file = "capitalism/monopsony_monopoly_rents.pdf", width = 9, height = 7)
...
WageFn <- function(h, ubar = 2.5, B = 2, t = 0.8) {
  B + ubar + (ubar - ubar*t +  ubar*t*h )/(t*(1 - h))
}

Mch <- function(h, ubar = 2.5, B = 2, t = 0.8) {
  B + ubar + (ubar - ubar*t +  ubar*t*h )/(t*(1 - h))+ (ubar*h)/(t*(1-h)^2)
}

AvgRevenue <- function(h, rmax = 40, xmax = 1.5){
  rmax - (rmax/xmax)*h
}

MRevenue <- function(h, rmax = 40, xmax = 1.5){
  rmax - 2*(rmax/xmax)*h
}
...
wagelvls <- c(WageFn(as.numeric(eq1[1])), MRevenue(as.numeric(eq1[1])), AvgRevenue(as.numeric(eq1[1])))
...
ticksy <- c(0, wagelvls, 40)
ylabels <- c(0, expression(paste(w[b])), expression(paste(w[m])), expression(paste(w[a])), NA)
ticksx <- c(0, LPoints, 1, xlims[2])
xlabels <- c(0, expression(paste(h[m])), expression(paste(h[a])), expression(paste(h[b])),  1.0, NA)
...
text(0.2, wagelvls[2] + 4,  expression(paste("Market")), xpd = TRUE, cex = labelsize) 
text(0.2, wagelvls[2] + 2,  expression(paste("rents")), xpd = TRUE, cex = labelsize)

text(0.85, wagelvls[1] - 1,  expression(paste(ac)), xpd = TRUE, cex = labelsize)
text(0.85, wagelvls[1] - 3,  expression(paste("(no monopsony)")), xpd = TRUE, cex = labelsize)
...
#ARPl, no monopoly
segments(0, wagelvls[3], xlims[2], wagelvls[3], lty = 1, lwd = graphlinewidth + 0.2, col = CBCols[3])
segments(LPoints[1], 0, LPoints[1], wagelvls[3], lty = 2, lwd = segmentlinewidth, col = grays[22])

#ACL no monopsony
segments(0, wagelvls[1], xlims[2], wagelvls[1], lty = 1, lwd = graphlinewidth + 0.2, col = CBCols[4])
