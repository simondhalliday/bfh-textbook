require(shape)
pdf(file = "competitionmarkets/coord_brfs_marginfig.pdf", width = 9, height = 7)
...
lines(xx2, PEC(xx2, alpha = 30, beta = 1/2), col = COL[2], lty = 2, lwd = graphlinewidth)
...
text(0.5*xlims[2], -2.5, expression(paste("A's hours, ", h^A)), xpd = TRUE, cex = axislabelsize)
text(-3.8, 9, expression(paste("B's hours, ", h^B)), xpd = TRUE, cex = axislabelsize, srt = 90) 
...
text(4.5, 12.4, expression("PEC"), cex = annotatesize)
...
#B's brf
text(3, 15.3, expression(paste("B's BRF")), cex = annotatesize)
#text(3, 15.0, expression(paste("function")), cex = labelsize)

#A's brf
text(15.9, 3.4, expression(paste("A's BRF")), cex = annotatesize, xpd = TRUE)
