...
df$pop <- c(2, 1, 2, 1, 2, 1, 2, 1, 2, 1, 2, 1) #Baselines
df$pop1 <- c(2, 1, 2, 2, 1, 1, 2, 1, 2, 1, 2, 1) #Step 1
df$pop2 <- c(2, 2, 2, 2, 1, 1, 1, 1, 2, 1, 2, 1) #Step 2
df$pop3 <- c(2, 2, 2, 2, 1, 1, 1, 1, 1, 1, 2, 2) #Step 3 - end
df$pop6 <- c(2, 2, 1, 1, 2, 2, 1, 1, 2, 2, 1, 1) #Ideal
...
ggsave("society/schelling_1_new.pdf", plot = p1, width = 6, height = 6)


#Step 2
p2 <- df %>% 
  ggplot(aes(x = x, y = y, 
             color = as.factor(pop2))) + 
  geom_circle(aes(x0 = 0, y0 = 0, r = 1), inherit.aes = FALSE) +
  geom_point(size = 7) +
  geom_curve(
    aes(x = -0.86, y = -0.5, xend = 0.5, yend = 0.86),
...
ggsave("society/schelling_2_new.pdf", plot = p2, width = 6, height = 6)

#Step 3
p3 <- df %>% ggplot(aes(x = x, y = y, 
               color = as.factor(pop3))) + 
  geom_circle(aes(x0 = 0, y0 = 0, r = 1), inherit.aes = FALSE) +
  geom_point(size = 7) +
  geom_curve(
    aes(x = 1, y = -0.0244, xend = -0.0183, yend = -1),
    arrow = arrow(length = unit(0.03, "npc"), type = "closed", end = "both"), col = "black"
  ) +
  scale_color_manual(values = c("#377EB8", "#4DAF4A")) + 
  theme_minimal() +
...
ggsave("society/schelling_3_new.pdf", plot = p3, width = 6, height = 6)
...
ggsave("society/schelling_4_new.pdf", plot = p4, width = 6, height = 6)

p5.2 <- df %>% ggplot(aes(x = x, y = y, color = as.factor(pop4))) + 
  geom_circle(aes(x0 = 0, y0 = 0, r = 1), inherit.aes = FALSE) +
  geom_point(size = 7) +
  geom_curve(
    aes(x = -0.5, y = -0.86, xend = -0.86, yend = 0.5),
...
ggsave("society/schelling_4_new_no_arrow.pdf", plot = p4, width = 6, height = 6)
