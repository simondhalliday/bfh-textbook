...
require(shape)
require(pBrackets)
pdf(file = "competitionmarkets/duopoly_residual.pdf", width = 9, height = 7)
...
ticksx <- c(0, 8/3, mrxInt(xB = 2.667) + 8/3,  12, xlims[2])
xlabels <- c(NA,  NA, expression(paste(frac(bar(p) - beta*x^B,2*beta ))), expression(paste(frac(bar(p),beta))), NA )
ticksy2 <- c(ylims[1], ylims[2])
  
axis(1, at = ticksx, pos = 0, labels = FALSE, cex.axis = axislabelsize)
text(x = c(0, mrxInt(xB = 2.667) + 8/3,  12, xlims[2]), par("usr")[3] - 0.4, labels = xlabels, srt = 0, pos = 1, xpd = TRUE, cex = axislabelsize)
axis(2, at = ticksy, pos = 0, labels = ylabels, las = 1, cex.axis = axislabelsize, col = "grey")
axis(2, at = ticksy2, pos = 8/3, labels = NA, las = 1, cex.axis = axislabelsize)
...
#Label the axes
text(0.5*(xlims[2]), -2.2, expression(paste("Firm A's output, ", x^A)), xpd = TRUE, cex = axislabelsize) 
#mtext(expression(paste("Quantity of output, ", x)), side=1, line = 2.5, cex = axislabelsize)
text(-2.4, 0.5*ylims[2], expression(paste("Price, Revenue and Costs, ($)")), xpd = TRUE, srt = 90, cex = axislabelsize) 
...
text(10/2 + 8/3, 10.9, expression(paste("A's residual demand")), cex = annotatesize, xpd = TRUE)
text(10/2 + 8/3, 10.2, expression(paste("given B producing ", x^{BN})), cex = annotatesize, xpd = TRUE)
