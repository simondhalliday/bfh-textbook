...
require(plotrix)
require(pBrackets)
pdf(file = "capitalism/lewis_development.pdf", width = 9, height = 7)
...
ticksy <- c(0, wagelvls[1], wagelvls[2], prodlvls, ylims[2])
ylabels <- c(0, expression(paste(w[1]^C)), expression(paste(w[2]^C)), expression(paste(gamma[1])), expression(paste(gamma[2])), NA)
ticksx <- c(0, emplvls, 1)
xlabels <- c(0, expression(paste(H[1]^N)), expression(paste(H[2]^N)), 1)
...
brackets(x1 = 1.02, y1 = 29.8, x2 = 1.02, y2 = 20.2,  
         ...
         lty = 1, xpd = TRUE)
text(1.14, 25, expression(paste("Profits, ", pi^A)), cex = labelsize, xpd = TRUE)
