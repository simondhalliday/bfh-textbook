require(shape)
pdf(file = "what_can_markets_do/sequential_coase_restricted_solved.pdf", width = 9, height = 7)
...
Arrows(2.5, 5, 4, 1, col = rgb(0, 0, 1, 0.5), lty = 1, lwd = 3, arr.type = "triangle")

#Branches for Anders
#Plant early
Arrows(5, 9, 2.5, 5, col = rgb(1, 0, 0, 0.5), lty = 1, lwd = 3, arr.type = "triangle")


#From Anders plants late
#Bianca plants early
#segments(7.5, 5, 6, 1, lty = 1, col = rgb(1, 0, 0, 0.5), lwd = graphlinewidth)
#Arrows(7.5, 5, 6, 1, col = rgb(1, 0, 0, 0.5), lty = 1, lwd = 3, arr.type = "triangle")

#Bianca plants late
#segments(7.5, 5, 9, 1, lty = 1, col = rgb(1, 0, 0, 0.5), lwd = graphlinewidth)

#Anders plays Don't Bargain
#segments(5, 9, 7.5, 5, lty = 1, col = rgb(0, 0, 1, fadelevel), lwd = graphlinewidth)
Arrows(5, 9, 7.5, 5, col = rgb(1, 0, 0, 0.5), lty = 1, lwd = 3, arr.type = "triangle")
...
#P2 Terminal Nodes
points(1, 1, pch = 16, col = rgb(0,0,0,fadelevel), cex = nodesize)
...
text(7.1, 7.5, expression(paste("Don't")), cex = actionlabelsize)
text(7.1, 7.1, expression(paste("bargain")), cex = actionlabelsize)
text(7.1, 6.7, expression(paste("(restrict)")), cex = actionlabelsize)

text(1.7, 5, expression(paste("Anders")), cex = labelsize)
text(1.3, 3.5, expression(paste("Accept")), cex = actionlabelsize, col = rgb(0,0,0,fadelevel))
#text(1.3, 3.1, expression(paste("early")), cex = actionlabelsize)
text(3.8, 3.5, expression(paste("Reject")), cex = actionlabelsize)
...
#text(6.1, 3.5, expression(paste("Restrict")), cex = actionlabelsize)
#text(6.1, 3.1, expression(paste("early")), cex = actionlabelsize, col = rgb(0,0,0,fadelevel))
#text(9.25, 3.5, expression(paste("Use")), cex = actionlabelsize, col = rgb(0,0,0,fadelevel))
#text(9.25, 3.1, expression(paste("unrestricted")), cex = actionlabelsize, col = rgb(0,0,0,fadelevel))

#Payoffs 
text(1, .6, expression("(" * phantom("4.5") * "," * phantom("1.5") * ")"), cex = actionlabelsize, col = rgb(0,0,0,fadelevel))
text(1, .6, expression(phantom("(") * "4.5" * phantom(",") * phantom("1.5") * phantom(")")), col = rgb(1, 0, 0, 1, fadelevel),  cex = actionlabelsize)
text(1, .6, expression(phantom("(") * phantom("4.5") * phantom(",") * "1.5" * phantom(")")), col = rgb(0, 0, 1, 1, fadelevel), cex = actionlabelsize)
...
#Cutting branch plant late when A plants
segments(2.4, 3.9, 1.9, 4.4, lty = 1, col = rgb(0,0,0,fadelevel), lwd = graphlinewidth)
segments(2.2, 3.25, 1.7, 3.75, lty = 1, col = rgb(0,0,0,fadelevel), lwd = graphlinewidth)
segments(2, 2.65, 1.5, 3.15, lty = 1, col = rgb(0,0,0,fadelevel), lwd = graphlinewidth)
segments(1.8, 2.05, 1.3, 2.55, lty = 1, col = rgb(0,0,0,fadelevel), lwd = graphlinewidth)
segments(1.6, 1.45, 1.1, 1.95, lty = 1, col = rgb(0,0,0,fadelevel), lwd = graphlinewidth)
