require(shape)
pdf(file = "bfh-textbook/constrained_optimization/offer_curve_cd.pdf", width = 9, height = 7)
...
#lines(xx1, PriceLine(xx1, w = 10, p = 0.25), col = COLB[3], lwd = graphlinewidth)
lines(xx1, PriceLine(xx1, w = 10, p = 0.5), col = COLB[3], lwd = graphlinewidth)
lines(xx1, PriceLine(xx1, w = 10, p = 2), col = COLB[3], lwd = graphlinewidth)
...
text(-1.4, 0.5*ylims[2], expression(paste("Quanity of oranges, ", y)), xpd = TRUE, cex = axislabelsize, srt = 90) 
...
text(11.8, 2.4, expression(u[2]))
text(11.8, 4.6, expression(u[3]))
#text(6.6, 8.3, expression(u[4]^A))

text(11.2, 5.25, expression("Offer Curve"))
...
segments(2.5, 5, xlims[2], 5, lty = 1, col = COL[3] , lwd = graphlinewidth)
