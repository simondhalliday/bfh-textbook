...
require(shape)
require(plotrix)
pdf(file = "bfh-textbook/property/property_fig4a.pdf", width = 9, height = 7)
...
xpoly1 <- c(0, 0, 12.24745, 0)
ypoly1 <- c(0, 12.24745, 0, 0)
...
polygon(x = xpoly2, y = ypoly2, col=COL[3], density=NULL, border = NA)
...
#Worse than UPF
text(4, 4, expression(paste("Welfare", phantom() < bar(W))))
text(4, 3.5, expression(paste("worse than UPF")))
text(4, 3, expression(paste("obtainable with existing ", bar(x), " and ", bar(y))))
text(4, 2.5, expression(paste("but Pareto-inefficient")))

#Better than UPF
text(9, 9, expression(paste("Welfare", phantom() > bar(W))))
text(9, 8.5, expression(paste("better than UPF")))
text(9, 8, expression(paste("but unobtainable with ")))
text(9, 7.5, expression(paste("existing ", bar(x), " and ", bar(y))))
...
#points(4, 5.12, pch = 16, col = "black", cex = 1.5)
#text(4.2, 5.3, expression(e))
