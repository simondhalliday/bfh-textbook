...
png(file = "firmmarketsupply/perf_comp_costs_outlier.png", width = 6*120, height = 4.5*120)
...
ylabels <- c(NA, expression(paste(p[c] == 6)), NA, NA)
ticksx <- c(0, 4, NA, xlims[2])
xlabels <- c(NA, expression(paste(q[c] == 4)), NA, NA)
...
text(-1.5, 0.5*ylims[2], expression(paste("Price per unit, revenues, and costs ($)")), xpd = TRUE, cex = axislabelsize, srt = 90) 

#Label curves

text(10, 5.5, expression(paste("ar = mr = p")), cex = labelsize)
text(11, 9, expression(paste(ac(q[i]))), cex = labelsize)
text(8.75, 11.5, expression(paste(mc(q[i]))), cex = labelsize)
...
text(4, 8.9, expression(paste("Profit maximum at")), cex = labelsize)
text(4, 8.3, expression(paste(mr == mc)), cex = labelsize)
shape::Arrows(4, 8, 4, 6.4, col = "black", lty = 1, lwd = 2, arr.type = "triangle", arr.lwd = 0.5)
