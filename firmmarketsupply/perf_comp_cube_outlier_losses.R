...
png(file = "firmmarketsupply/perf_comp_costs_cube_outlier_losses.png", width = 6*120, height = 4.5*120)
...
CBColsAlpha <- c(rgb(213/255, 94/255, 0, 0.5), rgb(204/255, 121/255, 167/255, 0.5))
...
ticksy <- c(0, MCost(eqmQ[1]), AvgCost(eqmQ[1]), NA, ylims[2])
ylabels <- c(NA, expression(paste(p[c]^{L} == 4.75)), round(AvgCost(eqmQ[1]),0), NA, NA)
ticksx <- c(0, eqmQ[1], NA, xlims[2])
xlabels <- c(NA, expression(paste(q[c]^{L} == 5.8)), NA, NA)
...
xpoly1 <- c(0, 0, eqmQ[1], eqmQ[1], 0)
ypoly1 <- c(0, MCost(eqmQ[1]), MCost(eqmQ[1]), 0, 0)
polygon(x = xpoly1, y = ypoly1, col=COLB[1],density=NULL, border = NA)

xpoly2 <- c(0, 0, eqmQ[1], eqmQ[1], 0)
ypoly2 <- c(MCost(eqmQ[1]), AvgCost(eqmQ[1]), AvgCost(eqmQ[1]), MCost(eqmQ[1]), MCost(eqmQ[1]))
polygon(x = xpoly2, y = ypoly2, col= CBColsAlpha[1], density=NULL, border = NA)
...
#Label the costs
text(3, 0.85*AvgCost(x = eqmQ[2]), expression("Losses"), cex = labelsize)
...
text(10, MCost(eqmQ[1]) - 0.5, expression(paste("ar = mr = p")), cex = labelsize)
...
# )
segments(eqmQ[1], 0, eqmQ[1], AvgCost(eqmQ[1]),
         lty = 2, col = grays[20] , lwd = segmentlinewidth
)
segments(0, AvgCost(eqmQ[1]), eqmQ[1], AvgCost(eqmQ[1]),
         lty = 2, col = grays[20] , lwd = segmentlinewidth
)

segments(0, MCost(eqmQ[1]), xlims[2], MCost(eqmQ[1]),
         lty = 1, col = CBCols[2] , lwd = graphlinewidth
)

# Label Points for comparison

points(eqmQ[1], MCost(eqmQ[1]), pch = 16, col = "black", cex = 1.5)
...
text(eqmQ[1] + 2, 2.9, expression(paste("Profit maximum at")), cex = labelsize)
text(eqmQ[1] + 2, 2.3, expression(paste(mr == mc)), cex = labelsize)
shape::Arrows(eqmQ[1] + 1.5, 3.2, eqmQ[1] + 0.2, MCost(eqmQ[1]) - 0.2, col = "black", lty = 1, lwd = 2, arr.type = "triangle", arr.lwd = 0.5)
