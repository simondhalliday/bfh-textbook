...
require(shape)
pdf(file = "specprodexch/ppf_dos_indiff_a.pdf", width = 8, height = 8)
...
ticksy <- c(ylims[1], ppf2(x = 7.15), 5.65,  8, ylims[2])
ylabels <- c(NA,  expression(paste(y[s]) == 3.6), expression(paste(y[d]) == 5.7),  expression(paste(bar(y) == 8)), NA)
ticksx <- c(xlims[1], 4.5, 5.65, 7.15, 8, xlims[2])
xlabels <- c(NA,NA, expression(paste(x[d] == 5.7)), expression(paste(x[s] == 7.2)), expression(paste(bar(x) == 8)), NA)
...
#Draw the polygon for shading the improvement 
xpoly2 <- c(16.95/2, 17.95/2, 0, 0)
ypoly2 <- c(0, 0, 17.95, 16.95)
...
lines(xx1, bcA(xx1, w = 16.95, p = 2), col = CBCols[3], lwd = graphlinewidth + 0.2)
...
text(4, 10.2, expression("Price lines"), cex = labelsize, xpd = TRUE)
#text(4, 10.6, expression("(feasible frontier with"), cex = labelsize, xpd = TRUE)
#text(4, 10.2, expression("production at d & s and exchange)"), cex = labelsize, xpd = TRUE)
#Arrows(8.5, 6, 8.5, 1.25, col = "black", lty = 1, lwd = 2, arr.type = "triangle", arr.lwd = 0.5)
...
text(9.5, 3.05, expression(u[1]^A), cex = labelsize)
