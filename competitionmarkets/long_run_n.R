...
Profit <- function(n, pmax = 20, c = 10, s = 0.5) {
  (pmax - c)^2 / (((n + 1)^2)*s)
}
...
totaln <- function(pi, pmax = 20, c = 10, s = 0.5){
  (pmax - c)/(sqrt(pi*s)) - 1
}

Supply <- function(x, c = 0, c2 = 0.05){
  c + 2*c2*x
}
...
bte <- function(n, b, pmax = 20, c = 10, s = 0.5) {
  Profit(n)*(1-b)
}
...
lines(xx1, bte(n = xx1, b = 0.7, c = 10), col = COLA[3], lty = 2, lwd = segmentlinewidth)
lines(xx1, bte(n = xx1, b = 0.5, c = 10), col = COLA[3], lty = 2, lwd = segmentlinewidth)
lines(xx1, bte(n = xx1, b = 0.2, c = 10), col = COLA[3], lty = 2, lwd = segmentlinewidth)


lines(xx1, Profit(xx1, c = 10), col = COLA[4], lwd = graphlinewidth) # No Barrier --- Green
...
segments(0, 3.2, xlims[2], 3.2, lty = 1, col = COLB[4] , lwd = segmentlinewidth)
...
# Label Demand
text(118, 3, expression(paste("Price and ")), cex = labelsize)
text(118, 2, expression(paste("Cost")), cex = labelsize)
