...
png(file = "firmmarketsupply/profit_costs_outlier.png", width = 7*120, height = 5*120)
...
ylabels <- c(NA, expression(paste(mc == c)), expression(paste(ac, "(", q[m],")")), expression(paste(ar, "(", q[m],")", phantom() == p[m])), NA)
ticksx <- c(0, 4, xlims[2])
xlabels <- c(NA, expression(paste(q[m])), NA)
...
text(-2, 0.5*ylims[2], expression(paste("Price per unit, revenue, and costs ($) ")), xpd = TRUE, cex = axislabelsize, srt = 90) 

#Label curves
text(10.5, 4.9, expression(paste(ac(q))), cex = labelsize)
text(10.5, 3.6, expression(paste(mc == c)), cex = labelsize)
text(10.5, 0.8, expression(paste(ar(q))), cex = labelsize)
text(4.75, 0.8, expression(paste(mr(q))), cex = labelsize)

#Labels cost and profit areas
text(1.5, 0.5*AvgCost(x = 4), expression("Total costs"), cex = annotatesize)
