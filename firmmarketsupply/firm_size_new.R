...
require(shape)
#library(matlib)
pdf(file = "firmmarketsupply/firm_size_new.pdf", width = 9, height = 7)
...
ticksy <- c(ylims[1], firmDeath(x = 4.2), firmGrowth(x = 4.2), firmGrowth(xsol),  firmGrowth(x = 10), firmDeath(x = 10), ylims[2])
ylabels <- c(expression(-f*underline(s)), expression(paste(d[a*minute])), expression(paste(g[a])), expression(paste(g[b] == d[b])), expression(paste(g[c])), expression(paste(d[c*minute])), NA)
ticksx <- c(xlims[1], 4.2, xsol, 10, xlims[2])
xlabels <- c(NA, expression(paste(bar(s)[a])), expression(paste(bar(s)[b])), expression(paste(bar(s)[c])), NA)
...
lines(xx1, firmGrowth(xx1), col = CBCols[1], lwd = graphlinewidth)
#lines(xx1, firmGrowth(xx1, s1 = 1.5), col = CBCols[2], lwd = graphlinewidth, lty=2)
...
text(14.3, firmGrowth(x=14.3) - 5, expression(bar(s)(1 - f)*g^s),cex = labelsize)
...
text(10 - 0.2, firmDeath(10) + 0.7, expression(paste(c*minute)),cex = labelsize)
...
text(4.2 + 0.4, firmDeath(4.2) - 0.2, expression(paste(a*minute)),cex = labelsize)
