...
library(shape)
pdf(file = "people/ample_millmart_expected_payoffs.pdf", width = 10, height = 8)
...
ticksx <- c(0, 2/5, 1/2, 1)
xlabels <- c(0, expression(paste(p == 2/5)), expression(paste(p == 1/2)), 1)
...
text(x = c(0, 2/5, 1/2, 1), par("usr")[3] - 0.2, labels = xlabels, srt = 30, pos = 1, xpd = TRUE)
...
text(xlims[2]*0.5, -1, expression(paste("Fraction playing don't, P")), xpd = TRUE, cex = axislabelsize)
...
segments(2/5, 0, 2/5, expected_dont(2/5), lty = 2, col = "gray", lwd = segmentlinewidth)
segments(0, expected_dont(2/5), 2/5, expected_12hours(12/27), lty = 2, col = "gray", lwd = segmentlinewidth)
points(2/5, expected_dont(2/5), pch = 16, col = "black", cex = 1.5)
...
text(0.8, 4, expression(paste(pi[dont] == 3 + P)), xpd = TRUE, cex = labelsize)
text(0.8, 2.2, expression(paste(pi[pay] == 5 - 4*P)),  xpd = TRUE, cex = labelsize)

#Axis arrow
arrows(28, -3.5, 35, -3.5, xpd = TRUE, length=0.1,angle=40,lwd=3)

#Risk Dominant Strategy
text(.5, 5, expression(paste("Strategy with higher")), xpd = TRUE, cex = labelsize)
text(.5, 4.7, expression(paste("expected payoff at ", p == 1/2)), xpd = TRUE, cex = labelsize)
text(.5, 4.4, expression(paste("is risk-dominant")), xpd = TRUE, cex = labelsize)
Arrows(.5, 4.2, .5, expected_dont(0.5) + 0.15, col = "black", lty = 1, lwd = 2, code = 2, arr.type = "triangle", arr.lwd = 0.5)

Arrows(.5, expected_dont(0.5) - 0.15, 0.98, expected_dont(0.98) -0.15, col = "black", lty = 1, lwd = 2, code = 2, arr.type = "triangle", arr.lwd = 0.5)
text(.8, 3.4, expression(paste("All players end up")), xpd = TRUE, cex = labelsize)
text(.8, 3.1, expression(paste("playing 'Don't'")), xpd = TRUE, cex = labelsize)
