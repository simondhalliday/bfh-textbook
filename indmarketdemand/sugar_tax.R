...
xlabels <- c(0, expression(paste(x["m"]) == 2),expression(paste(x[t]) == 4), expression(paste(x[nt]) == 5), expression(paste(x[max]==10)), NA)
...
# DWL
xpoly2 <- c(4, 5, 4) 
ypoly2 <- c(9, 10, 12) # Change y1 to 10 to meet gray line
...
text(-1.6, 0.5*ylims[2], expression(paste("Price per liter, ", p[x])), xpd = TRUE, cex = axislabelsize, srt = 90) 
...
text(8.5, 19, expression(paste("Market price with tax, ", p + t == 12)), cex = labelsize)
