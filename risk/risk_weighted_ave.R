...
text(-6.5, 0.5*ylims[2], expression(paste("Expected utility, v(y)")), xpd = TRUE, cex = axislabelsize, srt = 90) 
...
text(pointbx, 28.4 , expression(paste("Low probability, ", underline(p),",")), cex = labelsize)
...
text(pointcx - 1.5, 37.2 , expression(paste("means a v(L) closer to good")), cex = labelsize)
