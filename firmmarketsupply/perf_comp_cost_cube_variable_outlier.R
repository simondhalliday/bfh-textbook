...
png(file = "firmmarketsupply/perf_comp_costs_cube_variable_outlier.png", width = 6*120, height = 4.5*120)
...
VarAvgCost <- function(x, c0 = 20, c1 = 6, c2 = -1, c3 = 0.1){
  #  TotCost(x) / x
 c1 + c2*x + c3*(x^2)
}
...
eq4 <- uniroot(function(x)  MCost(x) - VarAvgCost(x)  , c(.01,1000), tol=1e-8)   

eqmQ <- as.numeric(c(eq1[1], eq2[1], eq3[1], eq4[1]))
...
ticksy <- c(0, VarAvgCost(eqmQ[4]), AvgCost(eqmQ[2]), NA, ylims[2])
ylabels <- c(NA, expression(paste(p[c]^S == 3.5)), expression(paste(p[c] == 6.75)), NA, NA)
ticksx <- c(0, eqmQ[4], eqmQ[2], NA, xlims[2])
xlabels <- c(NA, expression(paste(q[c]^S == 5)), expression(paste(q[c] == 7)), NA, NA)
...
#Label the costs
text(3, 0.3*AvgCost(x = eqmQ[2]), expression("Total costs"), cex = labelsize)
...
lines(xx1, VarAvgCost(xx1), col = CBCols[3], lwd = graphlinewidth)
...
text(2, AvgCost(eqmQ[2]) + 0.3, expression(paste("ar = mr = p")), cex = labelsize)
text(11, 9.75, expression(paste(ac(q[i]))), cex = labelsize)
text(11, 6.25, expression(paste(avc(q[i]))), cex = labelsize)
...
segments(eqmQ[4], 0, eqmQ[4], VarAvgCost(eqmQ[4]),
         lty = 2, col = grays[20] , lwd = segmentlinewidth
)
segments(0, VarAvgCost(eqmQ[4]), eqmQ[4], VarAvgCost(eqmQ[4]),
         lty = 2, col = grays[20] , lwd = segmentlinewidth
)
...
points(eqmQ[4], VarAvgCost(eqmQ[4]), pch = 16, col = "black", cex = 1.5)
...
text(eqmQ[2] + 2, 1.9, expression(paste("Shutdown point")), cex = labelsize)
text(eqmQ[2] + 2, 1.3, expression(paste(p == avc)), cex = labelsize)
shape::Arrows(eqmQ[2] + 0.7, 2.1, eqmQ[4] + 0.2, VarAvgCost(eqmQ[4]) - 0.2, col = "black", lty = 1, lwd = 2, arr.type = "triangle", arr.lwd = 0.5)
