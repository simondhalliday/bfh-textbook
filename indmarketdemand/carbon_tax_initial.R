require(shape)
pdf(file = "indmarketdemand/carbon_tax_initial.pdf", width = 9, height = 7)
...
ticksy <- c(0, 22, ylims[2])
#ticksy_mod <- c(10)
#ylabels_mod <- c(expression(paste(m)))
ylabels <- c(NA, expression(paste(m)), NA)
ticksx <- c(0, 6.25, 10, xlims[2])
xlabels <- c(NA, expression(paste(bar(x)[1])), expression(paste(bar(x)[2])), NA)
...
lines(xx1, bcA(xx1, p = 3.5), col = COLB[5], lwd = graphlinewidth)
lines(xx1, bcA(xx1, p = 1.75), col = COLB[3], lwd = graphlinewidth)
