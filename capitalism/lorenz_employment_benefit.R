...
library(shape)
library(pBrackets)
pdf(file = "employment/lorenz_employment_benefit.pdf", width = 8, height = 8)
...
ticksy <- c(ylims[1], 0.05, 0.6, ylims[2])
ylabels <- c(0, expression(paste(5)), expression(paste(60)), 100)
...
ypoly1 <- c(0, 0.05, 0.6, 1, 0)
polygon(x = xpoly1,
        y = ypoly1,
        col = COLB[1], density = NULL, border = NA)
#Area B
xpoly2 <- c(0, 0.1, 1, 1, 0.9, 0.1)
ypoly2 <- c(0, 0, 0, 1, 0.6, 0.05)
polygon(x = xpoly2,
        y = ypoly2,
        col = COLA[1], density = NULL, border = NA)

# Lorenz curve
segments(0, 0, 0.1, .05, lty = 1, col = COLA[5], lwd = graphlinewidth, xpd = TRUE)
segments(0.1, 0.05, 0.9, 0.6, lty = 1, col = COLA[5], lwd = graphlinewidth)
...
segments(0.1, 0, 0.1, 0.05, lty = 2, col = "gray", lwd = segmentlinewidth)
segments(0, 0.05, 1, 0.05, lty = 2, col = "gray", lwd = segmentlinewidth)
