require(shape)
require(plotrix)
pdf(file = "employment/noshirking_wage.pdf", width = 9, height = 7)
...
Fn1 <- function(h, ubar = 0.5, t = 1 ) {
  WageFn <- function(H, delta = 5) {
    delta /(1 - H) + (ubar*h)/t*(1-h)^2
  }
...
lines(xx1, Fn1(xx1), col = COLA[4], lwd = graphlinewidth)
