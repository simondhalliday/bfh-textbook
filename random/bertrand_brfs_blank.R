require(shape)
pdf(file = "random/bertrand_brfs_blank.pdf", width = 9, height = 7)
...
text(-4.3, 0.5*ylims[2], expression(paste("Price of Pixels, ", p[2])), xpd = TRUE, cex = axislabelsize, srt = 90) 
# 
...
# text(30, 18, expression(paste(p[2]^"*",(p[1])==frac(1,2)*(1 + p[1] +c[2]))), xpd = TRUE, cex = annotatesize) 
# 
# text(18, 36, expression(paste("iPhone BRF")), xpd = TRUE, cex = annotatesize) 
# text(18, 33, expression(paste(p[1]^"*",(p[2])==frac(1,2)*(1 + p[2] +c[1]))), xpd = TRUE, cex = annotatesize) 
