require(shape)
pdf(file = "coordination_failures/coord_brfs_only_cournot_step1.pdf", width = 9, height = 7)
...
ticksy <- c(0, ylims[2])
ylabels <- c(NA, NA)
ticksx <- c(0,  intercept1(alpha = 30), xlims[2])
xlabels <- c(NA,  expression(paste(frac(alpha, 1 + 2*beta))), NA)

axis(1, at = ticksx,  pos = 0, labels = FALSE)
text(x = c(0, intercept1(alpha = 30), xlims[2]), par("usr")[3] - 0.4, labels = xlabels, srt = 0, pos = 1, xpd = TRUE)
