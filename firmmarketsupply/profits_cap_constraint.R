...
require(shape)
pdf(file = "firmmarketsupply/profit_cap_constraint.pdf", width = 9, height = 7)
...
ylabels <- c(NA, expression(paste(mc == c[1])), expression(paste(ac(x[1]))), expression(paste(ar(x[1]))), NA)
...
text(-2.2, 0.5*ylims[2], expression(paste("Revenue and Costs, ",r, " and ", tc)), xpd = TRUE, cex = axislabelsize, srt = 90) 
...
segments(0, AvgCost(x = 4), 4, AvgCost(x = 4), lty = 2, col = "gray" , lwd = segmentlinewidth)
