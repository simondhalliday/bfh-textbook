require(shape)
pdf(file = "firmmarketsupply/supply_stepwise_firmi.pdf", width = 9, height = 7)
...
xlabels <- c(NA, expression(paste(underline(x)^I)), NA)
...
segments(shares[2], costs[1], shares[2], ylims[2], lty = 1, col = COLB[4] , lwd = segmentlinewidth)
...
# segments(0, 3, 3, 3, lty = 2, grays[20] , lwd = segmentlinewidth)
# segments(3, 0, 3, 3, lty = 2, grays[20] , lwd = segmentlinewidth)

#For market price
# segments(0, 2, 1, 2, lty = 2, grays[20] , lwd = segmentlinewidth)

#Label axes
#mtext(expression(paste("Quantity of output, ", x)), side=1, line = 2.5, cex = axislabelsize)
text(0.5*xlims[2], -0.3, expression(paste("Firm I's output, ", x^I)), xpd = TRUE, cex = axislabelsize) 
...
text(2.3, 2.8, expression("Firm I's"), cex = labelsize)
text(2.3, 2.65, expression("supply"), cex = labelsize)
...
segments(0, costs[3], xlims[2], costs[3], lty = 2, col = grays[20], lwd = segmentlinewidth)
