...
library(shape)
pdf(file = "risk/palanpur_risk_dom_bina.pdf", width = 10, height = 8)
...
text(xlims[2]*0.5, -3.5, expression(paste("Probability Bina will play early, P")), xpd = TRUE, cex = axistitlesize)
text(-3.5, 0.5*ylims[2], expression(paste("Expected payoff")), xpd = TRUE, cex = axistitlesize, srt = 90) 
...
text(7.8, 11, expression(paste("Plant Early")),  xpd = TRUE, cex = labelsize)

#Axis arrow
arrows(30, -3.5, 35, -3.5, xpd = TRUE, length=0.1,angle=40,lwd=3)
...
text(28.5, 35, expression(paste("Equal expected")), xpd = TRUE, cex = labelsize)
text(28.5, 33.5, expression(paste("payoffs at ", P == 2/3)), xpd = TRUE, cex = labelsize)
