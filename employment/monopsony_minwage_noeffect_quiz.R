require(shape)
require(plotrix)
jpeg(file = "employment/monopsony_minwage_quiz_part2.jpg", width = 6*120, height = 4.5*120)
...
ticksy <- c(0, 13, WageFn(LPoints[1]),  Mch(LPoints[1]), 40)
ylabels <- c(0, NA, expression(paste(w[d])), expression(paste(w[g])), NA)
ticksx <- c(0,  LPoints[1], IPoints[2],  IPoints[3], 1, xlims[2])
xlabels <- c(0,  expression(paste(h[d])), expression(paste(h[e])), expression(paste(h[f])),  1.0, NA)
axis(1, at = ticksx, pos = 0, labels = xlabels, cex.axis = labelsize)
axis(2, at = ticksy, pos = 0, labels = ylabels, las = 1,cex.axis = labelsize)

text(-0.035, 13,  expression(paste(w[e]^min)), xpd = TRUE, cex = labelsize) 
...
text(0.86, 31, expression(paste(G)), cex = labelsize, xpd = TRUE)
...
text(0.64, 31, expression(paste(F)), cex = labelsize, xpd = TRUE)
...
text(0.23, 31, expression(paste(E)), cex = labelsize, xpd = TRUE)
...
text(IPoints[2] - 0.01, MRP(IPoints[2]) - 1, expression(paste(e)), cex = labelsize)
...
segments(IPoints[3], 0, IPoints[3], WageFn(IPoints[3]), lty = 2, lwd = segmentlinewidth, col = grays[20])
points(IPoints[3], WageFn(IPoints[3]), pch = 16, col = "black", cex = 1.5)
text(IPoints[3] + 0.01, WageFn(IPoints[3]) - 1.2, expression(paste(f)), cex = labelsize)
...
points(LPoints[1], Mch(LPoints[1]), pch = 16, col = "black", cex = 1.5)
text(LPoints[1], MRP(LPoints[1]) + 1, expression(paste(g)), cex = labelsize)


points(LPoints[1], WageFn(LPoints[1]), pch = 16, col = "black", cex = 1.5)
text(LPoints[1] + 0.02, WageFn(LPoints[1]) - 1, expression(paste(d)), cex = labelsize)
