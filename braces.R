...
#'
#' @param xfrom,xto,yfrom,yto, mid start and end points of braces. Direction of brace determined by from and to arguments.
...
#'
#' @author Weikai Chen, modified from the braces function written by Tim Gerrodette \email{tim.gerrodette@@noaa.gov}
#'
#' @examples
...
#' @export
#'
braces <- function(xfrom, xto, yfrom, yto, radius = 1, mid = 0.5, col = par("fg"), lty = par("lty"), lwd = par("lwd")) {
 ...
 sapply(1:length(xfrom), function(i) {
     ...
     if (vertical) {
         xmid <- (xfrom[i] + xto[i]) * 0.5
         ymid <- (yfrom[i] +yto[i]) * mid
         ...
         xmid <- (xfrom[i] + xto[i]) * mid
         ymid <- (yfrom[i] +yto[i]) * 0.5
         sx <- sign(xto[i] - xfrom[i])
         sy <- sign(yto[i] - yfrom[i])
