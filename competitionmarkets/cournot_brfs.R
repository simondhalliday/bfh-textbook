...
ylabels <- c(NA, expression(paste(x^{BN})), expression(paste( (p[max] - c[1])/2*s )), expression(paste( (p[max] - c[1])/s )), NA)
ticksx <- c(0, 12, 18, 36, xlims[2])
xlabels <- c(NA, expression(paste(x^{AN})), expression(paste( (p[max] - c[1])/2*s )), expression(paste( (p[max] - c[1])/s )), NA)
