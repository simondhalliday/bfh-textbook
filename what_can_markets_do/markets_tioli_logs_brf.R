...
pdf(file = "what_can_markets_do/markets_tioli_logs_brf.pdf", width = 9, height = 7)
...
#a <- c(uA(9,2) - 1.5, uA(5,5), uA(8,2), uA(8,2) + 1)

#a2 <- c(uAlog(9,1), uAlog(2.91,12), uAlog(5,5), uAlog(10 - xbtioli(uBlog(9,1)), 15 - (3/2)*xbtioli(uBlog(9,1))))
a2 <- c(uAlog(2.91,12), uAlog(5,5), uAlog(5.5, 3))
...
#lines(xx1, indifflogA(xx1, U = uA(5, 5)), col = COLA[3], lwd = graphlinewidth)
...
#Price through j
lines(xx1, PriceLine(xx1, intercept = 10, slope = 1), col = CBCols[6], lwd = graphlinewidth)

#Price through k
lines(xx1, PriceLine(xx1, intercept = 6.18, slope = 0.58), col = CBCols[6], lwd = graphlinewidth)
...
text(2.35, 12, expression(u[2]^A), cex = annotatesize)
text(3.2, 12, expression(u[3]^A), cex = annotatesize)
#text(6.5, 12, expression(u[3]^A), cex = annotatesize)

#Label the prices
text(0.5, 5.3, expression(p[k]), cex = annotatesize)
text(0.5, 9, expression(p[j]), cex = annotatesize)
text(0.5, 13.2, expression(p[n]), cex = annotatesize)
...
# #Add a point for the initial endowment
points(10 - 4.8275, 15 - 7.25, pch = 16, col = "black", cex = 1.5)
text(5, 7.5, expression(n), cex = labelsize)


#point j
points(5, 15 - 5, pch = 16, col = "black", cex = 1.5)
text(5.2, 15 - 4.75, expression(j), cex = labelsize)

#point k
points(10 - 5.4, 15 - 3.1, pch = 16, col = "black", cex = 1.5)
text(10 - 5.5 + 0.2, 15 - 2.75, expression(k), cex = labelsize)
