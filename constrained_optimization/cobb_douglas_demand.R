...
library(shape)
pdf(file = "constrained_optimization/cobb_douglas_demand.pdf", width = 8, height = 8)
...
demand <- function(x, alpha = 0.5, endowment = 100){
  (endowment*alpha)/x
}
...
#Axis labels
mtext(expression(paste("Quantity of good x, ", x)), side = 1, line = 2.5, cex = axislabelsize)
text(-1.9, 0.5*ylims[2], expression(paste("Price of good x, ", p[x])), xpd = TRUE, cex = axislabelsize, srt = 90) 
