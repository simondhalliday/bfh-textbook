...
library(shape)
require(shape)
pdf(file = "risk/figure4_4.pdf", width = 10, height = 8)
...
ticksx <- c(0, 10, 25.6, 33, 39, 40)
xlabels <- c(NA, expression(paste(y - Delta*y[2])), expression(paste(y)), expression(paste(y[ce])), expression(paste(y + Delta*y[1])), NA)
...
segments(10, ConvexU(10), 39, ConvexU(39), lty = 1, col = COLB[4] , lwd = graphlinewidth)
...
text(25.6-0.6, ConvexU(25.7)+1.1, expression(paste("b")), cex = labelsize)
...
text(-2.7, ConvexU(10), expression(paste(v(y - Delta*y[2]))),  xpd = TRUE,  cex = labelsize)
text(-1.5, ConvexU(25.6), expression(paste(u(y))),  xpd = TRUE,  cex = labelsize)
