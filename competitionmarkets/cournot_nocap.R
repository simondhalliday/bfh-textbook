...
require(shape)
pdf(file = "competitionmarkets/cournot_nocap.pdf", width = 9, height = 7)
...
ylabels <- c(NA, expression(paste(c[1])), expression(paste(p^{oli})), expression(paste(p[max] - gamma)))
ticksx <- c(0, 6, 14, xlims[2])
xlabels <- c(NA, expression(paste(x^{oli})), expression(paste(x^{cap})), NA)
...
segments(14, 0, 14, ylims[2], lty = 1, col = COLC[7] , lwd = graphlinewidth)
...
text(6, 7.8, expression(paste("Note:")), cex = labelsize)
text(6, 7.45, expression(paste(gamma == s*(n-1)*bar(x))), cex = labelsize)
segments(4.5, 7.2, 7.5, 7.2, lty = 1, col = "black" , lwd = 1)
segments(4.5, 8, 7.5, 8, lty = 1, col = "black" , lwd = 1)
segments(4.5, 7.2, 4.5, 8, lty = 1, col = "black" , lwd = 1)
segments(7.5, 7.2, 7.5, 8, lty = 1, col = "black" , lwd = 1)

text(11.8, 7.8, expression(paste("Capacity Constraint")), cex = labelsize)
text(11.8, 7.5, expression(paste("does not bind;")), cex = labelsize)
text(11.8, 7.2, expression(paste("the firm produces ", x^{oli})), cex = labelsize)
