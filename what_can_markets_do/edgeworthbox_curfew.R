...
uA <- function(x, y, alpha = 1/4, Ta = 0) {
  y - alpha*(Ta - x)^2
}
...
uB <- function(x, y, beta = 1/4, Tb = 8) {
  -(y) - beta*(Tb - x)^2
}



indiffB <- function(x, utility = 11, beta = 1/2, Tb = 8) {
  utility + beta*((8 - x) - Tb)^2
}
...
axis(1, at = ticksx, pos = -12, labels = NA)
