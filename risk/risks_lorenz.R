...
library(shape)
library(pBrackets)
pdf(file = "risk/risks_lorenz.pdf", width = 10, height = 8)
...
ticksx <- seq(from = 0, to = ylims[2], by = 0.25)
xlabels <- seq(from = 0, to = ylims[2], by = 0.25)
...
#Area B
xpoly2 <- c(0, 1, 1, 0.5, 0)
ypoly2 <- c(0, 0, 1, 0.25, 0)
polygon(x = xpoly2,
        y = ypoly2,
...
text(0.8, 0.6, expression(paste(Lorenz)), cex = labelsize)
text(0.8, 0.56, expression(paste(curve)), cex = labelsize)
