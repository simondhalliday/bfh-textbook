require(shape)
pdf(file = "indmarketdemand/qql_worksheet.pdf", width = 9, height = 7)
...
ticksy <- c(0, 98, 110, 140, 150, 190, ylims[2])
ylabels <- c(NA, expression(paste(y,"*")), expression(paste(y == u[1])), expression(paste(y == m)), expression(paste(y == u[2])), expression(paste(y == u[3])), NA)
ticksx <- c(0, 6, 20, xlims[2])
...
text(-2.3, 0.5*ylims[2], expression(paste("Quanity of money, ", y)), xpd = TRUE, cex = axislabelsize, srt = 90) 
...
uM <- function(eM, eN){
  10*eM - 0.5*eM*eN - eM^2
}

uN <- function(eM, eN){
  10*eN - 0.5*eM*eN - eN^2
}
