...
pdf(file = "employment/employment_mrp_effort_nonlinear2_CP.pdf", width = 10, height = 8)
par(mar =  c(5, 7, 3, 3))
...
ticksx <- c(xlims[1], 667, 1000, 2000, xlims[2])
xlabels <- c(NA, expression(paste(h[1]^{N^'*'})), expression(paste(h[1]^{N}==1000)), expression(paste(h[2]^{N}==2000)), NA)
ticksy <- c(ylims[1], 20,30, ylims[2])
ylabels <- c(NA, expression(paste(w^N == 20)), expression(paste(w^N^'*' == 30)), NA)
...
arrows(950, 15, 700, 15, col = "black", lty = 1, lwd = 2, arr.type = "triangle", arr.lwd = 0.5)
