#require(ggplot2)
require(shape)
pdf(file = "what_can_markets_do/qql_eb1.pdf", width = 9, height = 7)
...
indiffcurveA1 <- function(x, uA = 14, rmax = 2.5, xmax = 10) {
  uA - rmax*x + (1/2)*(rmax/xmax)*(x^2)
}

indiffcurveA2 <- function(x, uA = 12.5, rmax = 2.5, xmax = 10) {
  uA - rmax*x + (1/2)*(rmax/xmax)*(x^2)
}

indiffcurveA3 <- function(x, uA = 15.5, rmax = 2.5, xmax = 10) {
  uA - rmax*x + (1/2)*(rmax/xmax)*(x^2)
}
...
plot(0, 0, xlim = xlims, ylim = ylims, type = "n",
     xlab = expression(paste("A's Good, ", x)),
     ylab = expression(paste("A's Money, ", y)), 
     line = 2.5,
     xaxt = "n", 
     yaxt = "n", 
...
text(-0.8, 7, expression(paste("B's Money, y")), xpd = TRUE, cex = axislabelsize, srt = 270) 
...
indiffcurveB3 <- function(x, U = 6.123724, A = 1, a = 0.5) {
  ((((U)/A)*(1/x)^a)^(1/(1-a)))
}
...
lines(xx1, indiffcurveB4(xx1), col = COLB[2], lwd = graphlinewidth)
