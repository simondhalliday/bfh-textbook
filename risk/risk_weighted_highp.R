...
ticksx <- c(0, 2, 36, xlims[2])
xlabels <- c(NA, expression(paste(y + delta[2])), expression(paste(y + delta[1])), NA)
...
text(-6.2, 0.5*ylims[2], expression(paste("The utility of wealth, u(y)")), xpd = TRUE, cex = axislabelsize, srt = 90) 
...
text(36.8, ConcaveU(36)-.9, expression(paste("b")), cex = labelsize)
