...
pdf(file = "risk/risk_income_indiffs_quiz.pdf", width = 8, height = 8)
...
ppf <- function(x, xbar = 25, slope = 1/100, bary = 4) {
  bary - slope *(xbar-x)^2
}

uFn <- function(x, y, xbar = 25, alpha = 0.4){
  ((xbar-x)^alpha)*(y^(1-alpha))
}

# indiffA <- function(x, ubar = uFn(8,3) - 1, xbar = 25, alpha = 0.4) {
#   (ubar/ ((xbar-x)^alpha))^(1/ (1 - alpha))
# }


#These functions are for calculating alternative values for the 
yval <- function(x, s = 1/100, xbar = 25, alpha = 0.4) {
  (2*s*(1 - alpha))/alpha*(xbar - x)^2
}

xbarxsq <- function(ybar, s = 1/100, alpha = 0.4) {
  ybar/(2*s*(1 - alpha)/alpha + s)
}

xval <- function(xbar, ybar = 4, s = 1/100, alpha = 0.4) {
  xbar - sqrt(ybar/(2*s*(1 - alpha)/alpha + s))
}
...
#final:
a <- c(uFn(15, 3) - 1, uFn(15, 3), uFn(15, 3) + 1.4) #alpha = 0.4
...
text(-2.1, 0.5*ylims[2], expression(paste("Expected income (1,000s), ", hat(y) )), xpd = TRUE, cex = axislabelsize, srt = 90) 
...
text(15.3, ppf(15) - 0.1, expression(paste(b)), cex = labelsize)
