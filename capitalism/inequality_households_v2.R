...
library(shape)
library(plotrix)
pdf(file = "capitalism/inequality_households_v2.pdf", width = 10, height = 8)
...
text(35, 4.6, expression(paste(y^C == 2)), xpd = TRUE, cex = labelsize)
...
text(34, 18, expression(paste(Delta^{AC} == phantom(), y^A - y^C == 8)), xpd = TRUE, cex = labelsize)
#Difference between A and B
text(6, 18, expression(paste(Delta^{AB} == phantom(), y^A - y^B == 7)), xpd = TRUE, cex = labelsize)
#Difference between B and C
text(20, 3, expression(paste(Delta^{BC} == phantom(), y^B - y^C == 1)), xpd = TRUE, cex = labelsize)
