require(shape)
pdf(file = "competitionmarkets/demand_side_econ_scale2.pdf", width = 9, height = 7)
...
#Label axes
mtext(expression(paste("Price per unit of x, ", p)), side=1, line = 2, cex = axislabelsize)
text(-2, 0.5*ylims[2],expression(paste("Market quantity of output, ", X)) , xpd = TRUE, cex = axislabelsize, srt = 90) 
# 
...
text(15, 10, expression("Demand curve"), cex = labelsize)
text(15, 8, expression(X == frac(paste(p(X) - underline(p)), beta)), cex = labelsize)
