...
library(shape)
library(pBrackets)
pdf(file = "capitalism/lorenz_competition_exam.pdf", width = 10, height = 8)
...
ticksy <- c(ylims[1], 0.6, 0.65, ylims[2])
ylabels <- c(0, expression(paste(60)), 65,  100)
...
# Lorenz curve
segments(0, 0, 0.1, 0, lty = 2, col = COLA[5], lwd = graphlinewidth)
segments(0.1, 0, 0.9, 0.6, lty = 2, col = COLA[5], lwd = graphlinewidth)
segments(0.9, 0.6, 1, 1, lty = 2, col = COLA[5], lwd = graphlinewidth)

segments(0, 0, 0.05, 0, lty = 1, col = COLA[5], lwd = graphlinewidth)
segments(0.05, 0, 0.9, 0.65, lty = 1, col = COLA[5], lwd = graphlinewidth)
segments(0.9, 0.65, 1, 1, lty = 1, col = COLA[5], lwd = graphlinewidth)


text(0.8, 0.69, expression(paste("New")), cex = labelsize)
text(0.8, 0.66, expression(paste("Lorenz")), cex = labelsize)
text(0.8, 0.63, expression(paste("curve")), cex = labelsize)

text(0.8, 0.49, expression(paste("Old")), cex = labelsize)
text(0.8, 0.46, expression(paste("Lorenz")), cex = labelsize)
...
segments(0.9, 0, 0.9, 0.65, lty = 2, col = "gray", lwd = segmentlinewidth)
segments(0, 0.6, 0.9, 0.6, lty = 2, col = "gray", lwd = segmentlinewidth)

#segments(0.9, 0, 0.9, 0.6, lty = 2, col = "gray", lwd = segmentlinewidth)
segments(0, 0.65, 0.9, 0.65, lty = 2, col = "gray", lwd = segmentlinewidth)
