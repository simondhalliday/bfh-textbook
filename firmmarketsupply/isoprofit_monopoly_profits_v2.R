...
require(shape)
pdf(file = "firmmarketsupply/isoprofit_monopoly_profits_v2.pdf", width = 9, height = 7)
...
text(x = c(0, 1.55, 4, 6.45, 12, xlims[2]), par("usr")[3] - 0.4, labels = xlabels, srt = 0, pos = 1, xpd = TRUE, cex = labelsize)
...
text(-1.5, 0.5*ylims[2], expression(paste("Price per unit ($)")), xpd = TRUE, cex = axislabelsize, srt = 90) 
