require(shape)
pdf(file = "bfh-textbook/coordination_failures/coord_responses_c.pdf", width = 9, height = 7)
...
uA <- function(ea, eb, alpha = 12, beta = 1/48) {
  alpha*(1 - beta*eb)*ea - 0.5*(ea)^2
}

uB <- function(ea, eb, alpha = 12, beta = 1/48) {
  alpha*(1 - beta*ea)*eb - 0.5*(eb)^2
}

brfA <- function(eb, alpha = 12, beta = 1/48) {
  alpha*(1 - beta*eb)
}

brfB <- function(eb, alpha = 12, beta = 1/48) {
  (alpha - eb)/(alpha * beta)
}
...
lines(xx1, brfA(xx1, alpha = 12, beta = 1/48), col = COLA[4], lwd = graphlinewidth)
lines(xx1, brfB(xx1, alpha = 12, beta = 1/48), col = COLB[4], lwd = graphlinewidth)
