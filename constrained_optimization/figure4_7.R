...
library(shape)
require(shape)
pdf(file = "/Users/rileyboeth/Library/Mobile Documents/com~apple~CloudDocs/bfh textbook - backup/coordination_failures/figure4_7a.pdf", width = 10, height = 8)
...
lines(xx1, indiffA(xx1, ua = 2), col = COLA[4], lwd = graphlinewidth)
lines(xx1, indiffA(xx1, ua = 8.2), col = COLA[4], lwd = graphlinewidth)
lines(xx1, indiffA(xx1, ua = 14.4), col = COLA[4], lwd = graphlinewidth)
lines(xx1, avgwealth(xx1), col = COLA[6], lwd = graphlinewidth)
...
text(8, avgwealth(8)+1, expression(paste("e*'")), cex = labelsize)
...
Arrows(-1, avgwealth(8)+6, -1, avgwealth(8)+1, col = "black", code =3, arr.type = "simple",xpd = TRUE)
Arrows(-1, avgwealth(8)-6, -1, avgwealth(8)-1, col = "black", code = 3, arr.type = "simple",xpd = TRUE)
