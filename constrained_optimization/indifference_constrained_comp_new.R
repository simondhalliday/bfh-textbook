...
library(shape)
pdf(file = "constrained_optimization/indifference_constrained_comp_new.pdf", width = 8, height = 8)
...
uFn <- function(x, y, alpha = 0.4, z = 2){
  ((x + z)^alpha)*((y + z)^(1-alpha))
}
...
a <- c(uFn(8, 3) - 3, uFn(8, 3)-2, uFn(8, 3) -1) #alpha = 0.4
