...
pdf(file = "employment/employment_mrp_effort_nonlinear_CP.pdf", width = 10, height = 8)
par(mar =  c(5, 7, 3, 3))
...
ticksx <- c(xlims[1], 333, 500, 1000, xlims[2])
xlabels <- c(NA, expression(paste(italic(l[1]^{N^'*'}))), expression(paste(italic(l[1]^{N}) == 500)), expression(paste(italic(l[2]^{N}) == 1000)),NA)
ticksy <- c(ylims[1], 40,60, ylims[2])
ylabels <- c(NA, expression(paste(c[italic(l)]^N== 40)), expression(paste(c[italic(l)]^N^'*'== 60)), NA)
...
arrows(500, 20, 350, 20, col = "black", lty = 1, lwd = 2, arr.type = "triangle", arr.lwd = 0.5)
