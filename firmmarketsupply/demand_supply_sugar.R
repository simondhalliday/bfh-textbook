...
require(shape)
pdf(file = "firmmarketsupply/demand_supply_sugar.pdf", width = 9, height = 7)
...
ylabels <- c(NA, expression(paste(p[a])), expression(paste(bar(p))), NA)
ticksx <- c(0, 75, 120, xlims[2])
xlabels <- c(NA, expression(paste(x[a])), expression(paste(bar(X))), NA)
...
#Label axes
mtext(expression(paste("Quantity of sugary drinks (liters), ", X)), side = 1, line = 3, cex = axislabelsize)
#mtext(expression(paste("Market output of the good, ", X)), side = 1, line = 3, cex = axislabelsize)
text(-12, 0.5*ylims[2], expression(paste("Price per liter ($), ", p)), xpd = TRUE, cex = axislabelsize, srt = 90) 
