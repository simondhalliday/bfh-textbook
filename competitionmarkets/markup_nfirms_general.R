require(shape)
pdf(file = "competitionmarkets/markup_nfirms_general.pdf", width = 9, height = 7)
...
lines(xx2, markUp(xx2, pmax = 20, c1 = 2), col = CBCols[1], lwd = graphlinewidth)
#lines(xx2, marketProfit(xx2, s = 1/2, pmax = 20, c1 = 2), col = COLB[4], lwd = graphlinewidth)

segments(0, 2, xlims[2], 2, lty = 1, col = CBCols[2] , lwd = graphlinewidth)
...
#Mark-up
text(3.2, 3.25, expression(paste("Markup")), cex = labelsize)
...
text(35, 3.2, expression(paste(p(n) == c + frac(1, n + 1)*(bar(p) - c))), cex = labelsize, xpd = TRUE)

text(35, 1.5, expression(paste("Marginal cost")), cex = labelsize)
text(35, 0.9, expression(paste(mc(x) == c)), cex = labelsize)
