...
library(shape)
pdf(file = "constrained_optimization/feasible_frontier_indiff_vegetables_bads.pdf", width = 8, height = 8)
...
ppf <- function(x, intercept = 320) {
  (intercept - 2*(sqrt(320/2) - x)^2)^0.5
}

uFn <- function(x, y, alpha = 0.4){
  ((sqrt(320/2) - x)^alpha)*(y^(1-alpha))
}

indiffA <- function(x, ubar = uFn(sqrt(320/2)-8, 13.87), alpha = 0.4) {
...
a <- c(uFn(sqrt(320/2)-8, 13.87) - 1, uFn(sqrt(320/2)-8, 13.87), uFn(sqrt(320/2)-8, 13.87) + 1.5) #alpha = 0.4
...
npts <- 500 
xx1 <- seq(xlims[1]+0.00001, xlims[2], length.out = npts)


#Draw the polygon for shading the feasible set
 xpoly1 <- seq(from = xlims[1], to = xlims[2], length.out = npts)
 ypoly1 <- ppf(xpoly1)
 polygon(x = c(0.001, xpoly1, 18, 18, 0.001), y = c(0.001, ypoly1, ppf(18), 0.001, 0.001), col=COLA[1], density=NULL, border = NA)
...
text(14, 14, expression("Feasible Frontier"), cex = labelsize)
Arrows(14, 14.5, 14, ppf(14) - 0.4, col = "black", lty = 1, lwd = 2, arr.type = "triangle", arr.lwd = 0.5)
...
#Axis labels
mtext(expression(paste("Work time (hours = max hours - free time), ", h)), side = 1, line = 2.5, cex = axislabelsize)
text(-1.6, 0.5*ylims[2], expression(paste("Crates of vegetables, ", y)), xpd = TRUE, cex = axislabelsize, srt = 90) 

#Add mrs = mrt at i
points(sqrt(320/2)-8, ppf(x = sqrt(320/2)-8), pch = 16, col = "black", cex = 1.5)
text(sqrt(320/2)-8, 5, expression(paste(mrs(x,y) == mrt(x,y))), cex = labelsize)
#text(sqrt(320/2)-8 + 0.05, 3.6, expression(paste(frac(2*y,3*x) == frac(2*x,y))), cex = labelsize)
Arrows(sqrt(320/2)-8, 5.2, sqrt(320/2)-8, ppf(x = sqrt(320/2)-8) - 0.6, col = "black", lty = 1, lwd = 2, arr.type = "triangle", arr.lwd = 0.5)


#Label the indifference curves
text(9.1, 19, expression(u[1]^A), cex = labelsize)
text(7.2, 19, expression(u[2]^A), cex = labelsize)
text(5.2, 19, expression(u[3]^A), cex = labelsize)
