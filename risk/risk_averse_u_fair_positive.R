...
ticksy <- c(0, ConcaveU(2), ConcaveU(10.2), ConcaveU(19), ConcaveU(36), ylims[2])
ylabels <- c(NA, NA, NA, NA, NA, ylims[2])
ticksx <- c(0, 2, 10.2, 19, 36, xlims[2])
#xlabels <- c(NA, expression(paste(y^B == y - delta[2])), expression(paste(y[c])), expression(paste(y)), expression(paste(y^G == y + delta[1])), NA)
xlabels <- c(NA, NA, expression(paste(y[c])), expression(paste(y[0])), expression(paste(y^G == y[0] + delta[1])), NA)


text(2.5, -1.6, expression(paste(y^B == y[0] - delta[2])), xpd = TRUE, cex = labelsize)
...
text(19.8, ConcaveU(10.2)-.5, expression(paste("e")), cex = labelsize)
segments(0, ConcaveU(10.2), 19, ConcaveU(10.2), lty = 2, col = grays[20], lwd = segmentlinewidth)
points(19, ConcaveU(10.2), pch = 16, col = "black", cex = 1.5)

text(10.2 + 0.8, ConcaveU(10.2)-.75, expression(paste("c")), cex = labelsize)
segments(10.2, 0, 10.2, ConcaveU(10.2), lty = 2, col = grays[20], lwd = segmentlinewidth)
points(10.2, ConcaveU(10.2), pch = 16, col = "black", cex = 1.5)
...
text(-3.5, ConcaveU(36), expression(paste(u(y[0] + delta[1]))), xpd = TRUE, cex = labelsize)

text(-4.2, ConcaveU(10.2) + 2.5, expression(paste(u(y[c]) == v(L), phantom() == phantom() )),  xpd = TRUE, cex = labelsize)
text(-5, ConcaveU(10.2), expression(paste(p%.%u(y[0] +  delta[1]) + phantom())),  xpd = TRUE, cex = labelsize)
text(-5.5, ConcaveU(10.2) - 2.5, expression(paste((1 - p)%.%u(y[0] - delta[2]))),  xpd = TRUE, cex = labelsize)
#text(-1.8, ConcaveU(9.7) - 3, expression(paste(phantom() == v(L))),  xpd = TRUE, cex = labelsize)
text(-3.5, ConcaveU(2), expression(paste(u(y[0] - delta[2]))),  xpd = TRUE,  cex = labelsize)
text(-2.3, ConcaveU(19), expression(paste(u(y[0]))),  xpd = TRUE,  cex = labelsize)

#Add risk premium distance arrow and label

Arrows(10.9, ConcaveU(2), 19 - 0.7, ConcaveU(2), col = "black", lty = 1, lwd = 2, code = 3, arr.type = "triangle", arr.lwd = 0.5)
text((19 + 10.2)/2, ConcaveU(2) + 1.5, expression(paste("Risk premium")),  xpd = TRUE,  cex = labelsize)
