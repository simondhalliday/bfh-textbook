...
pdf(file = "risk/eb_risk_insurance_100.pdf", width = 9, height = 7)
...
uA <- function(x, y, slopeA = 20, slopeInc = 0.5) {
  slopeInc*(y)^2 - slopeA*(x)^2
}

# uA <- function(x, y, slopeA = 12, c = 5) {
#   y - (x*slopeA)^c
# }

#u^W = X^W - ((1-\delta)*4)^2

uB <- function(x, y, slopeB = 0.5, slopeIncB = 0.5) {
  slopeIncB*(y)^2 - slopeB*(x)^2
}
...
ticksx <- seq(from = xlims[1], to = xlims[2], by = 10)
xlabels <- seq(from = xlims[1], to = xlims[2], by = 10)
...
uB2 <- function(x, y, slopeB = 0.5, slopeIncB = 0.5) {
  50 + slopeIncB*(y)^2 - slopeB*(x)^2
}
#b <- c(uB2(16, 0),  uB2(16,0) + 254)
b <- c(uB(0,50), uB(0.5,75))
