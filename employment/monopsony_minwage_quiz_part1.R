require(shape)
require(plotrix)
jpeg(file = "employment/monopsony_minwage_quiz_part1.jpg", width = 6*120, height = 4.5*120, units = "px", quality = 100)
...
ticksy <- c(0, WageFn(LPoints[1]), Mch(IPoints[2]), Mch(LPoints[1]), 40)
ylabels <- c(0, expression(paste(w[b])),expression(paste(w[d])), expression(paste(w[a])),  NA)
ticksx <- c(0,  LPoints[1], IPoints[2],  1, xlims[2])
xlabels <- c(0,  expression(paste(h[a] == h[b] )), expression(paste(h[c] == h[d] )),  1.0, NA)
axis(1, at = ticksx, pos = 0, labels = xlabels, cex.axis = labelsize)
axis(2, at = ticksy, pos = 0, labels = ylabels, las = 1,cex.axis = labelsize)

text(-0.035, 7.7,  expression(paste(w[c]^min)), xpd = TRUE, cex = labelsize) 


#Axis labels 
text(0.5*xlims[2], -3.5,  expression(paste("Hours of employment as a fraction of local labor supply, ", h)), xpd = TRUE, cex = axislabelsize) 
text(-0.1, 0.5*ylims[2], expression(paste("Wages, costs, and revenues ($)")), xpd = TRUE, cex = axislabelsize, srt = 90) 
...
text(0.86, 31, expression(paste(C)), cex = labelsize, xpd = TRUE)
...
text(0.64, 31, expression(paste(B)), cex = labelsize, xpd = TRUE)
...
text(0.23, 31, expression(paste(A)), cex = labelsize, xpd = TRUE)
...
#Nonsense line 
segments(0, Mch( IPoints[3]), IPoints[3], Mch( IPoints[3]), lty = 2, lwd = segmentlinewidth, col = grays[20])

#Second nonsense line 
segments(0,Mch(LPoints[1]) , LPoints[1], Mch(LPoints[1]) , lty = 2, lwd = segmentlinewidth, col = grays[20])

#Binding minimum wage
segments(0, 7.8, IPoints[3], 7.8, lty = 1, lwd = graphlinewidth, col = COL[2])
segments(IPoints[3], 7.8, 1, 7.8, lty = 2, lwd = graphlinewidth, col = COL[2])

#Green part of min wage
segments(0, 7.8 - 0.3, IPoints[3], WageFn(IPoints[3]) - 0.3, lty = 1, lwd = graphlinewidth, col = COLA[4])

#Segment of discontinuous mch
segments(IPoints[3], 7.8, IPoints[3], Mch(IPoints[3]), lty = 2, lwd = graphlinewidth, col = COLA[3])


segments(IPoints[2], 0, IPoints[2], WageFn(IPoints[2]), lty = 2, lwd = segmentlinewidth, col = grays[20])
...
text(IPoints[3] + 0.02, Mch(IPoints[3]), expression(paste(d)), cex = labelsize)
...
text(IPoints[3] + 0.015, WageFn(IPoints[3]) - 0.9, expression(paste(c)), cex = labelsize)
...
points(LPoints[1], Mch(LPoints[1]), pch = 16, col = "black", cex = 1.5)
text(LPoints[1], MRP(LPoints[1]) + 1, expression(paste(a)), cex = labelsize)


points(LPoints[1], WageFn(LPoints[1]), pch = 16, col = "black", cex = 1.5)
text(LPoints[1] + 0.02, WageFn(LPoints[1]) + 1.1, expression(paste(b)), cex = labelsize)
