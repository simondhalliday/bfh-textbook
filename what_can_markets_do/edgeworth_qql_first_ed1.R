...
arrows(6.3, -1, 9, -1, xpd = TRUE, length = 0.1, angle = 40, lwd = 3)
...
#Add arrows:
arrows(-0.6, 6.6, -0.6, 9, xpd = TRUE, length=0.1,angle=40,lwd=3)
arrows(2.7, -1.2, 4.5, -1.2, xpd = TRUE, length=0.1,angle=40,lwd=3)
...
text(9.65, 0.6, expression(u[1]^A))
text(9.65, 2.8, expression(u[2]^A))

#Label the indifference curves for B
text(1, 9, expression(u[1]^B))
text(1, 6.8, expression(u[2]^B))
...
text(6.6, -1, expression(paste("Quantity of good x")), xpd = TRUE)
text(6.6, -1.4, expression(paste("A sells to B")), xpd = TRUE)

brackets(x1 = 10.2, y1 = 4.4, x2 = 10.2, y2 = 0.9,  
         ticks = 0.5, curvature = 0.5, type = 1, 
         col = "black", lwd = 2, lty = 1, xpd = TRUE)
text(11.2, 2.6, expression(paste("Quantity of good y")), xpd = TRUE, srt = 270)
