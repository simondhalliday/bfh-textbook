...
uFn <- function(x, y, alpha = 1){
  ((x)^alpha)*(y^(1-alpha))
}

indiffA <- function(x, ubar = uFn(10,0) - 1, alpha = 1) {
...
text(0.5*xlims[2] - 0.1, -0.9, expression(paste("A's payoff (dollars), ", pi^A)), xpd = TRUE, cex = axislabelsize) 
text(-0.9, 0.5*ylims[2], expression(paste("B's payoff (dollars), ", pi^B)), xpd = TRUE, cex = axislabelsize, srt = 90) 

#Add mrs = mrt at i
# text(7, 4, expression(paste(mrs == mrt)), cex = labelsize)
# Arrows(7, 3.85, 7, 3.3, col = "black", lty = 1, lwd = 2, arr.type = "triangle", arr.lwd = 0.5)

#Label the indifference curves
text(8.7, 10.5, expression(u[1]^A), cex = labelsize)
text(9.7, 10.5, expression(u[2]^A), cex = labelsize)
text(10.7, 10.5, expression(u[3]^A), cex = labelsize)

#Annotate max u point on feasible frontier
text(10.2, 0.2, expression(paste(b)), cex = labelsize)
...
points(10, 0, pch = 16, col = "black", cex = 1.5, xpd = NA)

# text(4.2+0.2, ppf(4.2) + 0.2, expression(paste(a*minute)), cex = labelsize)
# segments(4.2, 0, 4.2, ppf(x = 4.2), lty = 2, col = "gray", lwd = segmentlinewidth)
# segments(0, ppf(x = 4.2), 4.2, ppf(x = 4.2), lty = 2, col = "gray", lwd = segmentlinewidth)
# points(4.2, ppf(x = 4.2), pch = 16, col = "black", cex = 1.5)
# 

# text(9+0.2, ppf(9) + 0.2, expression(paste(b*minute)), cex = labelsize)
# segments(9, 0, 9, ppf(x = 9), lty = 2, col = "gray", lwd = segmentlinewidth)
# segments(0, ppf(x = 9), 9, ppf(x = 9), lty = 2, col = "gray", lwd = segmentlinewidth)
