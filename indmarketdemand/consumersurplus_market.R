...
xlabels <- c(NA, expression(paste(X[m]) == 50), expression(paste(n*bar(x)==100)), NA)
...
text(98, 5.75, expression(paste("Inverse demand")), cex = labelsize)
text(98, 4, expression(paste(p(X) == 20 - frac(1,5)*X)), cex = labelsize,xpd = TRUE)
