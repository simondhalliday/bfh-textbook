require(ggplot2)
require(shape)
pdf(file = "capitalism/risk_contrast_selfemployed.pdf", width = 9, height = 7)
...
ticksx <- c(0, 5.6, 7.5, 12, xlims[2])
xlabels <- c(NA, expression(paste(Delta[A])), expression(paste(Delta[B])), expression(paste( bar(Delta) )), NA)
ticksy <- c(0, 4.4, 5.75, 7.1, riskreturn(5.6), riskreturn(7.5), riskreturn(g = 12), ylims[2])
ylabels <- c(NA, expression(paste(bar(w)[A])), expression(paste(w,"*")), expression(paste(bar(w)[B])), expression(paste(y[A])), expression(paste(y[B])), expression(paste(y(bar(Delta)))), NA)
...
text(9.75, 5, expression(paste("Wage from work, ", w,"*")), cex = labelsize)
