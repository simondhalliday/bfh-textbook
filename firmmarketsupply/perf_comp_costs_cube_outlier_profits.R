...
png(file = "firmmarketsupply/perf_comp_costs_cube_outlier_profits.png", width = 6*120, height = 4.5*120)
...
CBColsAlpha <- c(rgb(213/255, 94/255, 0, 0.5), rgb(204/255, 121/255, 167/255, 0.5), rgb(240/255, 228/255, 66/255, 0.5))
...
ticksy <- c(0,  AvgCost(eqmQ[3]), MCost(eqmQ[3]), NA, ylims[2])
ylabels <- c(NA,  round(AvgCost(eqmQ[3]),0), expression(paste(p[c]^{H} == 9)), NA, NA)
ticksx <- c(0, eqmQ[3], NA, xlims[2])
xlabels <- c(NA, expression(paste(q[c]^{H} == 7.9)), NA, NA)
...
xpoly1 <- c(0, 0, eqmQ[3], eqmQ[3], 0)
ypoly1 <- c(0, AvgCost(eqmQ[3]), AvgCost(eqmQ[3]), 0, 0)
polygon(x = xpoly1, y = ypoly1, col=COLB[1],density=NULL, border = NA)

xpoly2 <- c(0, 0, eqmQ[3], eqmQ[3], 0)
ypoly2 <- c(AvgCost(eqmQ[3]), MCost(eqmQ[3]), MCost(eqmQ[3]), AvgCost(eqmQ[3]), AvgCost(eqmQ[3]))
polygon(x = xpoly2, y = ypoly2, col= CBColsAlpha[3], density=NULL, border = NA)
...
#Label the costs
text(3, 0.65*AvgCost(x = eqmQ[3]), expression("Total costs"), cex = labelsize)
text(3, 0.9*MCost(x = eqmQ[3]), expression("Economic"), cex = labelsize)
text(3, 0.9*MCost(x = eqmQ[3]) - 0.5, expression("profits"), cex = labelsize)
...
lines(xx1, AvgCost(xx1), col = CBCols[4], lwd = graphlinewidth)
...
text(1.5, MCost(eqmQ[3]) + 0.3, expression(paste("ar = mr = p")), cex = labelsize)
...
# )
segments(eqmQ[3], 0, eqmQ[3], MCost(eqmQ[3]),
         lty = 2, col = grays[20] , lwd = segmentlinewidth
)
segments(0, AvgCost(eqmQ[3]), eqmQ[3], AvgCost(eqmQ[3]),
         lty = 2, col = grays[20] , lwd = segmentlinewidth
)

segments(0, MCost(eqmQ[3]), xlims[2], MCost(eqmQ[3]),
         lty = 1, col = CBCols[2] , lwd = graphlinewidth
)

# Label Points for comparison

points(eqmQ[3], MCost(eqmQ[3]), pch = 16, col = "black", cex = 1.5)
...
text(eqmQ[3] - 2, 10.9, expression(paste("Profit maximum at")), cex = labelsize)
text(eqmQ[3] - 2, 10.3, expression(paste(mr == mc)), cex = labelsize)
shape::Arrows(eqmQ[3] - 1.5, 10.3-0.3, eqmQ[3] - 0.2, MCost(eqmQ[3]) + 0.2, col = "black", lty = 1, lwd = 2, arr.type = "triangle", arr.lwd = 0.5)
