...
ylabels <- c(NA, expression(paste(1 - h^v)), expression(paste(1 - h[a])), expression(paste(f == 1)), NA)
ticksx <- c(0, 2, 2.8, 4, xlims[2])
xlabels <- c(NA, expression(paste(x[a] == w*h[a])), expression(paste(x^v == w*h^v)), expression(paste(x == w)), NA)
...
lines(xx1, bcA(xx1, w = 0.25, h = 1), col = CBCols[2], lwd = graphlinewidth)
...
text(4.6, 0.25, expression(paste(u[a])), cex = annotatesize)
...
segments(0, 0.3, 2.8, 0.3, lty = 2, col = grays[20] , lwd = segmentlinewidth)
segments(2.8, 0, 2.8, 0.3, lty = 2, col = grays[20] , lwd = segmentlinewidth)
