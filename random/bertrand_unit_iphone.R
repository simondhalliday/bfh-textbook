...
pdf(file = "random/bertrand_unit_iphone.pdf", width = 6, height = 3.5)
...
#      xpd = TRUE, srt = 0, cex = labelsize)
text(2, 0.9, labels=emoji("neutral_face"), family="OpenSansEmoji", cex=2.9)

brackets(x1 = 2, y1 = 1.03, x2 = 6, y2 = 1.03,  
         ticks = 0.5, curvature = 0.5, type = 1, 
         col = "black", lwd = 2, lty = 1, xpd = TRUE)
text(4, 1.2, expression(paste("Demand for Pixels")), 
     xpd = TRUE, srt = 0, cex = annotatesize)
# text(5, 1.23, expression(paste(Q[2](p[1],p[2])  == frac(1,2) - Delta*p)), 
#      xpd = TRUE, srt = 0, cex = annotatesize)
#== 1 - Q[1](p[1],p[2]), phantom()

brackets(x1 = 2, y1 = 0.83, x2 = 0, y2 = 0.83,  
         ticks = 0.5, curvature = 0.5, type = 1, 
         col = "black", lwd = 2, lty = 1, xpd = TRUE)
text(1, 0.67, expression(paste("Demand for iPhones")), 
     xpd = TRUE, srt = 0, cex = annotatesize)
# text(2, 0.555, expression(paste(Q[1](p[1],p[2]) == frac(1,2) + Delta*p)), 
#      xpd = TRUE, srt = 0, cex = annotatesize)


text(2.7, 1.48, expression(paste("When iPhones are more expensive than Pixels")), 
     xpd = TRUE, srt = 0, cex = annotatesize)
...
#ps = grid.export("bertrand_emoji.svg", addClass=T)
