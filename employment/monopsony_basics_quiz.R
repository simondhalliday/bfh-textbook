require(shape)
require(plotrix)
jpeg(file = "employment/monopsony_basics_quiz.jpg", width = 6*120, height = 4.5*120)
...
ticksy <- c(0, WageFn(LPoints[1]), MRP(0.61), MRP(LPoints[2]), MRP(LPoints[1]), 40)
ylabels <- c(0,  expression(paste(w[b])), expression(paste(w[d])), expression(paste(w[c])), expression(paste(w[a])), NA)
ticksx <- c(0, LPoints, 0.61, 1, xlims[2])
xlabels <- c(0, expression(paste(h[a] == h[b])), expression(paste(h[c])), expression(paste(h[d])),  1.0, NA)
...
text(0.89, 36, expression(paste(C)), cex = labelsize, xpd = TRUE)

#text(0.62, 43, expression(paste("Marginal cost")), cex = labelsize, xpd = TRUE)
text(0.66, 36, expression(paste(B)), cex = labelsize, xpd = TRUE)

#text(0.2, 43, expression(paste("Marginal revenue product")), cex = labelsize, xpd = TRUE)
text(0.2, 36, expression(paste(A)), cex = labelsize, xpd = TRUE)
...
segments(0, 12.5, 1, 12.5, lty = 2, lwd = segmentlinewidth, grays[20])
...
segments(0, WageFn(LPoints[1]), 1, WageFn(LPoints[1]), lty = 2, lwd = segmentlinewidth, col = grays[20])

#w[a] segment
segments(0, MRP(LPoints[1]), 1, MRP(LPoints[1]), lty = 2, lwd = segmentlinewidth, col = grays[20])


segments(LPoints[2], 0, LPoints[2], MRP(LPoints[2]), lty = 2, lwd = segmentlinewidth, col = grays[20])

#W[d] segment
segments(0, MRP(0.61), 1, MRP(0.61), lty = 2, lwd = segmentlinewidth, col = grays[20])
segments(0.61, 0, 0.61, MRP(0.61), lty = 2, lwd = segmentlinewidth, col = grays[20])
points(0.61, MRP(0.61), pch = 16, col = "black", cex = 1.5)
text(0.61 + 0.015, MRP(0.61) - 1.2, expression(paste(d)), cex = labelsize)
...
text(LPoints[1] + 0.02, WageFn(LPoints[1]) - 1, expression(paste(b)), cex = labelsize)
