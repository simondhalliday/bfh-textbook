require(ggplot2)
require(shape)
pdf(file = "information_power/information_mb_mc.pdf", width = 9, height = 7)
...
mc <- function(q, delta = 5) {
  (delta) / (1 - q)^2
}

mb <- function(q, p = 20, delta = 5) {
  (p - (delta) / (1 - q))*(1/(1-q))
}
...
lines(xx2, mb(xx2), col = COLA[4], lwd = graphlinewidth)
lines(xx2, mb(xx2, p = 15), col = COLA[4], lwd = graphlinewidth)
...
text(- 0.18, ylims[2] - 0.5*(ylims[2] - ylims[1]), expression(paste("Marginal benefit and marginal cost,", list(u[q], t[q]*v))), xpd = TRUE, cex = axislabelsize, srt = 90) 
