...
require(shape)
library(pBrackets)
pdf(file = "firmmarketsupply/monopoly_markup.pdf", width = 9, height = 7)
...
text(11.7, 4.5, expression(paste(mc(x) == c)), cex = annotatesize)
text(11.7, 2.2, expression(paste(p(x) == bar(p) - beta*x)), cex = annotatesize)
