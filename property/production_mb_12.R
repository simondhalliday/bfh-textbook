require(shape)
pdf(file = "coordination_failures/production_mb_12.pdf", width = 9, height = 12)
...
ticksy <- c(0, output(ea = 12), output(ea = 12, eb = 0), ylims[2])
ylabels <- c(NA, output(ea = 12), output(ea = 12, eb = 0), NA)
...
xx3 <- seq(8, 16, length.out = npts)

lines(xx1, output(xx1, eb = 0), col = COLA[3], lwd = graphlinewidth)
lines(xx2, output(xx2, eb = 12), col = COLA[6], lwd = graphlinewidth)
...
text(12 - 0.75, output(ea = 12) + 14, expression(paste(j)), cex = annotatesize)

points(12, output(ea = 12, eb = 0), pch = 16, col = "black", cex = 1.5)
text(12 - 0.75, output(ea = 12, eb = 0) + 12, expression(paste(k)), cex = annotatesize)
...
ticksy <- c(0, MBenefit(12, eb = 12), MBenefit(12, eb = 0), MBenefit(0, eb = 12), MBenefit(0, eb = 0), ylims[2])
ylabels <- c(NA, MBenefit(12, eb = 12), MBenefit(12, eb = 0), 24, 30, NA)
...
lines(xx1, MBenefit(xx1, eb = 0), col = COLA[3], lwd = graphlinewidth)

#Marginal benefit when B produce NE number of hours
lines(xx1, MBenefit(xx1, eb = 12), col = COLA[6], lwd = graphlinewidth)

segments(0, MBenefit(ea = 12), 12, MBenefit(ea = 12), lty = 2, col = grays[20], lwd = segmentlinewidth, xpd = 350)
segments(0, MBenefit(ea = 12, eb = 0), 12, MBenefit(ea = 12, eb = 0), lty = 2, col = grays[20], lwd = segmentlinewidth, xpd = 350)

segments(12, 0, 12, 500, lty = 2, col = grays[20], lwd = segmentlinewidth, xpd = 350)

text(20, 15.25, expression(paste("Marginal benefit")), cex = labelsize)
text(20, 13.75, expression(paste("when, ", h^B== 0 )), cex = labelsize)


text(4, 15.25, expression(paste("Marginal benefit ")), cex = labelsize)
text(4, 13.75, expression(paste("when, ", h^B == 12 )), cex = labelsize)

points(12, MBenefit(ea = 12), pch = 16, col = "black", cex = 1.5)
text(12 + 0.75, MBenefit(ea = 12) + 1, expression(paste(j)), cex = annotatesize)

# points(12, MBenefit(ea = 12), pch = 16, col = "black", cex = 1.5)
# text(12 + 0.75, MBenefit(ea = 12) + 1, expression(paste(j)), cex = annotatesize)

points(12, MBenefit(ea = 12, eb = 0), pch = 16, col = "black", cex = 1.5)
text(12 + 0.75, MBenefit(ea = 12, eb = 0) + 1, expression(paste(k)), cex = annotatesize)
