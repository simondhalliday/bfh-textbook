...
library(shape)
pdf(file = "firmmarketsupply/techvariables1.pdf", width = 8, height = 6)
...
#Draw the graphs
lines(xx1, isoA(xx1, alpha = 0.7, xbar = 4), col = COLA[5], lwd = graphlinewidth)
...
text(8, 4, expression(paste("Capital-intensive")), cex = labelsize)
text(8, 3.5, expression(paste("Technology A, ", x[1]^A)), cex = labelsize)

text(8, 1.85, expression(paste("Labor-intensive ")), cex = labelsize)
text(8, 1.35, expression(paste("Technology B, ", x[1]^B)), cex = labelsize)
