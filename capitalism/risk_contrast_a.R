require(ggplot2)
require(shape)
pdf(file = "capitalism/risk_contrast_a.pdf", width = 9, height = 7)
...
ylabels <- c(NA, expression(paste(bar(w)[a])), expression(paste(w^c )), expression(paste(hat(y)[a])), expression(paste(hat(y)*(bar(Delta)) )), NA)
...
text(8.1, 17, expression(u[2]^A), cex = labelsize)
text(9.7, 17, expression(u[1]^A), cex = labelsize)
#text(8.2, 17, expression(u[2]^A), cex = labelsize)



text(4, 16, expression(paste("Better for")), cex = labelsize)
text(4, 15, expression(paste("Ana")), cex = labelsize)
Arrows(4.5, 12.5, 1, 17,
