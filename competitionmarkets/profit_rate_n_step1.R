require(shape)
pdf(file = "competitionmarkets/profit_rate_n_step1.pdf", width = 9, height = 7)
...
ticksy <- c(0, 7, ylims[2])
ylabels <- c(NA,  expression(paste(rho[2])), NA)
ticksx <- c(0, 4.3, xlims[2])
xlabels <- c(NA, expression(paste(n[1])), NA)
...
points(4.3, 7, pch = 16, col = "black", cex = 1.5)
