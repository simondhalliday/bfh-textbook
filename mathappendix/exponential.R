...
library(shape)
pdf(file = "mathappendix/exponential.pdf", width = 8, height = 6)
...
xlabels <- c(NA, 2, 4, expression(x))
ticksy <- c(ylims[1], 20, 40, 60, ylims[2])
ylabels <- c(0, 20, 40, 60, expression(y))
...
# lines
lines(xx1, exp(xx1), col = COLB[4], lwd = graphlinewidth)

# points
#points(1.333, 10.333, pch = 16, col = "black", cex = 1.5)

# label function
text(4.5, 60, expression(paste("y = ", exp(x))),  xpd = TRUE, cex = labelsize)
