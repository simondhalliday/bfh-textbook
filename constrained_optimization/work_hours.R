...
library(ggplot2)
library(mosaic)
WorkHoursBase <- read.csv("/Users/shalliday/Google Drive/github/bfh-textbook/constrained_optimization/workhours.csv")
WorkHoursSelected <- 
  WorkHoursBase %>%
...
WorkHoursPlot1 %>% 
  ggplot(aes(x = year, y = hour, group = name, color = name, shape = name)) +
  geom_point() +
  geom_line() +
  ylim(1000, 3500) + 
