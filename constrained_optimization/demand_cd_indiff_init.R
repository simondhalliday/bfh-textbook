require(shape)
pdf(file = "constrained_optimization/demand_cd_indiff_init.pdf", width = 9, height = 7)
...
ylabels <- c(NA, expression(paste(y[b])), expression(paste(y == m )), NA)
...
text(12, -1, expression(paste(x == frac(m, p) )), cex = labelsize, xpd = TRUE)
...
text(0.5*xlims[2], -1.4,  expression(paste("Kilograms of fish, ", x)), xpd = TRUE, cex = axislabelsize) 
text(-1.2, 0.5*ylims[2], expression(paste("Money left over, ", y)), xpd = TRUE, cex = axislabelsize, srt = 90) 


#Label the iso-welfare functions for the HG, Aisha
text(11.8, 1.65, expression(u[1]), cex = annotatesize)
text(11.8, 3.3, expression(u[2]), cex = annotatesize)
text(11.8, 5.3, expression(u[3]), cex = annotatesize)
...
text(6.1, 6.5, expression(b), cex = annotatesize)
