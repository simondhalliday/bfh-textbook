...
xlabels <- c(NA, expression(paste(r[Low])), expression(paste(r[High])), NA)
...
text(-1.5, 0.5*ylims[2], expression(paste("Expected income, ", g)), xpd = TRUE, cex = axislabelsize, srt = 90) 
...
text(12, 17, expression(u[1]^A), cex = labelsize)
#text(10.1, 17, expression(u[2]^B), cex = labelsize)
text(10, 17, expression(u[1]^B), cex = labelsize)
