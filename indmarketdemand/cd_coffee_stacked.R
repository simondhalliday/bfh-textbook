...
library(shape)
pdf(file = "indmarketdemand/cd_coffee_stacked.pdf", width = 9, height = 14)
...
lines(xx1, bcA(xx1, w = 12, p = 1), col = CBCols[1], lwd = graphlinewidth)
lines(xx1, bcA(xx1, w = 12, p = 1.5), col = CBCols[1], lwd = graphlinewidth)
lines(xx1, bcA(xx1, w = 12, p = 3), col = CBCols[1], lwd = graphlinewidth)
...
abline(h=6, col=CBCols[3], lwd=graphlinewidth)
...
#Axis labels
text(0.5*xlims[2], -.6, expression(paste("Kilograms of coffee, ", x)), xpd = TRUE, cex = axislabelsize) 
...
text(10.5, 0.95, expression(paste("Demand curve")), cex = labelsize)
text(10.5, 0.75, expression(paste("for coffee")), cex = labelsize)
...
segments(demandx[1], 0, demandx[1], demand(demandx[1]) + 1.5, lty = 2, col = grays[20], lwd = segmentlinewidth, xpd = TRUE)
segments(0, demand(demandx[2]), demandx[2], demand(demandx[2]), lty = 2, col = grays[20], lwd = segmentlinewidth)
segments(demandx[2], 0, demandx[2], demand(demandx[2]) + 3.5, lty = 2, col = grays[20], lwd = segmentlinewidth, xpd = TRUE)
segments(0, demand(demandx[3]), demandx[3], demand(demandx[3]), lty = 2, col = grays[20], lwd = segmentlinewidth)
segments(demandx[3], 0, demandx[3], demand(demandx[3]) + 4.5, lty = 2, col = grays[20], lwd = segmentlinewidth, xpd = TRUE)
