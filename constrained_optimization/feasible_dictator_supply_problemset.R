...
library(shape)
pdf(file = "constrained_optimization/feasible_dictator_supply_problemset.pdf", width = 8, height = 8)
...
#Draw the graphs
lines(xx1, supply(xx1), col = COLA[5], lwd = graphlinewidth)
...
text(-0.9, 0.5*ylims[2], expression(paste("Marginal rate of transformation, ", mrt(pi^A, pi^B))), xpd = TRUE, cex = axislabelsize, srt = 90) 
...
text(8.5, 2.6, expression(paste("Supply of altruistic")), cex = labelsize)
text(8.5, 2.4, expression(paste("transfers to B")), cex = labelsize)
