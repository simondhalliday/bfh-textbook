require(shape)
pdf(file = "coordination_failures/coord_brfs_cournot_zoom.pdf", width = 9, height = 7)
...
uA <- function(ea, eb, alpha = 24, beta = 1/2) {
  (alpha - beta*(ea+eb))*ea - 0.5*(ea)^2
}

uB <- function(ea, eb, alpha = 24, beta = 1/2) {
  (alpha - beta*(ea+eb))*eb - 0.5*(eb)^2
}

brfB <- function(ea, alpha = 24, beta = 1/2) {
  (alpha - beta*ea)/(1 + 2*beta)
}

brfA <- function(ea, alpha = 24, beta = 1/2) {
  (alpha - ea*(1 + 2*beta))/(beta)
}

brfPE <- function(ea, alpha = 24, beta = 1/8) {
  alpha*(1 - beta*ea)
}
...
indiffeA <- function(ea, alpha = 24, beta = 1/2, uA = 46.08) {
  1/beta - (0.5*ea)/(alpha*beta) - uA/(alpha*beta*ea)
}

indiffB <- function(eb, alpha = 24, beta = 1/2, uB = 46.08) {
  (alpha*eb - uB - 0.5*eb^2)/(alpha*beta*eb)
}

indiffB2 <- function(ea, alpha = 24, beta = 1/2, uB = 46.08) {
  sqrt(alpha^2*(beta*ea - 1)^2 - 2*uB) - alpha*beta*ea + alpha
}

indiffB3 <- function(ea, alpha = 24, beta = 1/2, uB = 46.08) {
  -sqrt(alpha^2*(beta*ea - 1)^2 - 2*uB) - alpha*beta*ea + alpha
}
...
# indiffAlowB <- function(eb, alpha = 24, beta = 1/2, uA = 46.08) {
#   0.5*(-sqrt((2*alpha*beta*eb - 2*alpha)^2 - 8*uA)) - 2*alpha*beta*eb + 2*alpha
# }
# 
# indiffAlow <- function(ea, alpha = 24, beta = 1/2, uA = 46.08) {
#   0.5*(-sqrt((2*alpha*beta*ea - 2*alpha)^2 - 8*uA)) - 2*alpha*beta*ea + 2*alpha
# }
# 
# indiffAgain <- function(ea, alpha = 24, beta = 1/2, uA = 46.08) {
...
a <- c(uA(hANE(alpha = 24, beta = 1/2), hANE(alpha = 24, beta = 1/2)), 
       #uA(hApEff(alpha = 24), hApEff(alpha = 24)), 
       uA(hApEff2(alpha = 24, beta = 1/2), hApEff2(alpha = 24, beta = 1/2)))
b <- c(uA(hANE(alpha = 24, beta = 1/2),hANE(alpha = 24, beta = 1/2)), 
       #uB(hApEff(alpha = 24), hApEff(alpha = 24)), 
       uB(hApEff2(alpha = 24, beta = 1/2), hApEff2(alpha = 24, beta = 1/2)))
...
ticksy <- c(0, hApEff2(alpha = 24), hANE(alpha = 24), intercept1(alpha = 24), ylims[2])
ylabels <- c(NA, expression(paste(h^B,"*")), expression(paste(h^{BN})), expression(paste(frac(alpha, 1 + 2*beta))), NA)
ticksx <- c(0, hApEff2(alpha = 24), hANE(alpha = 24), intercept1(alpha = 24), xlims[2])
...
ypoly1 <- indiffA(xpoly1, uA = 46.08, alpha = 24, beta = 1/2)
ypoly2 <- indiffB3(xpoly1, uB = 46.08, alpha = 24, beta = 1/2)
polygon(x = c(xpoly1, rev(xpoly1)), y = c(ypoly1, rev(ypoly2)), col=COL[4], density=NULL, border = NA)



lines(xx1, brfA(xx1, alpha = 24, beta = 1/2), col = COLA[4], lwd = graphlinewidth)
lines(xx1, brfB(xx1, alpha = 24, beta = 1/2), col = COLB[4], lwd = graphlinewidth)
#lines(xx1, indiffA(xx1, alpha = 24, beta = 1/2, uA = 46.08), col = COL[2], lwd = graphlinewidth)
#lines(xx1, indiffB(xx1, alpha = 24, beta = 1/2, uB = 42), col = COL[2], lwd = graphlinewidth)
#lines(xx1, indiffB2(xx1, alpha = 24, beta = 1/2, uB = 46.08), col = COL[2], lwd = graphlinewidth)
#lines(xx1, indiffB3(xx1, alpha = 24, beta = 1/2, uB = 46.08), col = COL[2], lwd = graphlinewidth)
#lines(xx1, indiffAlow(xx1, uA = 42, alpha = 24, beta = 1/2), col = COL[2], lwd = graphlinewidth)
#lines(xx1, indiffAgain(xx1, uA = 42, alpha = 24, beta = 1/2), col = COL[2], lwd = graphlinewidth)
...
text(5.5, 9, expression(paste("B's hours, ", h^B)), xpd = TRUE, cex = axislabelsize, srt = 90) 
...
points(hANE(alpha = 24), hANE(alpha = 24), pch = 16, col = "black", cex = 1.5)
#text(11.3, 10.1, expression(paste("Nash Equilibrium")))
text(hANE(alpha = 24) + 0.3, hANE(alpha = 24) + 0.3, expression(paste("n")))
...
points(hApEff2(alpha = 24), hApEff2(alpha = 24), pch = 16, col = "black", cex = 1.5)
text(hApEff2(alpha = 24), hApEff2(alpha = 24) - 0.3, expression(paste(i)))
