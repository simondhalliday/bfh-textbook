...
library(pBrackets)
library(ineq)
pdf(file = "risk/halliday_inequality.pdf", width = 10, height = 8)
...
COLG <- c("#f0f0f0", "#d9d9d9", "#bdbdbd", "#969696", "#737373", "#525252", "#252525")
...
text(0.5*xlims[2], -0.1, expression(paste("Cumulative population proportion")), xpd = TRUE, cex = axislabelsize) 
...
polygon(x = xpoly2,
        y = ypoly2,
        col = COLG[4], density = NULL, border = NA)

#Area B
polygon(x = hhperc,
        y = inccum,
        col = COLG[2], density = NULL, border = NA)


# Lorenz curve
lines(x = hhperc, y = inccum,  lty = 1, col = "black", lwd = graphlinewidth)
...
#Line of equality
segments(0, 0, 1, 1, lty = 2, col = "black", lwd = graphlinewidth)
