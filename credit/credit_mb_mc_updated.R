library(shape)
library(plotrix)
pdf(file = "credit/credit_mb_mc_updated.pdf", width = 7, height = 7)
...
eqL <- uniroot(function(x)  mcFn(x) - mbFn(x, delta = deltalvl[1])  , c(.01,10), tol=1e-8)   
fls <- c(as.numeric(eqL[1]), as.numeric(eqN[1]), as.numeric(eqH[1]), 1.2 + deltalvl[1], 1.2 + deltalvl[2], 1.2 + deltalvl[3])
...
ticksy <- c(0, mcFn(f = fls[1]), mcFn(f = fls[2]), mcFn(f = fls[3]), fls[4], fls[5], fls[6], ylims[2])
ylabels <- c(0, expression(paste(mb[b])), expression(paste(mb[n])), expression(paste(mb[e])), expression(paste(q + delta[1])), expression(paste(q + delta[2])), expression(paste(q + delta[3])), NA)
...
ticksx <- c(0, eqL[1], eqN[1], eqH[1], 1, xlims[2])
xlabels <- c(0, expression(paste(f[b] )), expression(paste(f[n] )), expression(paste(f[e] )), 1.0, NA)
...
lines(xx1, mbFn(xx1, q = 1.2, delta = deltalvl[1]), col = CBCols[1], lwd = graphlinewidth)
...
text(0.15, 1.3, expression(paste(mb[1])), cex = labelsize)
text(0.15, 1.6, expression(paste(mb[2])), cex = labelsize)
...
points(fls[3], mcFn(fls[3]), pch = 16, col = "black", cex = 1.5)
...
text(fls[2], mcFn(fls[2]) + 0.075, expression(paste(n)), cex = labelsize)
