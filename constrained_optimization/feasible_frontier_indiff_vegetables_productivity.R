...
library(shape)
pdf(file = "constrained_optimization/feasible_frontier_indiff_vegetables_productivity.pdf", width = 8, height = 8)
...
a <- c(uFn(8, 13.87) - 1, uFn(8, 13.87), uFn(8, 13.87) + 1.3) #alpha = 0.4
...
npts <- 500 
xx1 <- seq(xlims[1], sqrt(400/2-0.001), length.out = npts)
xx2 <- seq(xlims[1], sqrt(320/2-0.001), length.out = npts)


#Draw the polygon for shading the feasible set
xpoly1 <- seq(from = xlims[1], to = sqrt(400/2-0.001), length.out = npts)
ypoly1 <- ppf(xpoly1)
polygon(x = c(xpoly1, xpoly1[1]), y = c(ypoly1, rev(ypoly1)[1]), col=COLA[2], density=NULL, border = NA)


xpoly2 <- seq(from = xlims[1], to = sqrt(320/2-0.001), length.out = npts)
ypoly2 <- ppf(xpoly2, intercept = 320)
polygon(x = c(xpoly2, xpoly2[1]), y = c(ypoly2, rev(ypoly2)[1]), col=COLA[1], density=NULL, border = NA)


#Draw the graphs
lines(xx1, ppf(xx1), col = COLA[5], lwd = graphlinewidth)
lines(xx2, ppf(xx2, intercept = 320), col = COLA[5], lwd = graphlinewidth)

#Label the feasible frontier
text(10.6, 1.1, expression("Original"), cex = labelsize)
text(10.6, 0.5, expression("Feasible Frontier"), cex = labelsize)
#Arrows(10.5, 0.53, 12.2, 0.53, col = "black", lty = 1, lwd = 2, arr.type = "triangle", arr.lwd = 0.5)

text(16.1, 1.1, expression("New"), cex = labelsize)
text(16.1, 0.5, expression("Feasible Frontier"), cex = labelsize)
#Arrows(10.5, 0.53, 12.2, 0.53, col = "black", lty = 1, lwd = 2, arr.type = "triangle", arr.lwd = 0.5)
...
#Add mrs = mrt at i
points(9, ppf(x = 9), pch = 16, col = "black", cex = 1.5)
text(9, 5, expression(paste(mrs(x,y) == mrt(x,y))), cex = labelsize)
text(9 + 0.05, 3.6, expression(paste(frac(2*y,3*x) == frac(2*x,y))), cex = labelsize)
Arrows(9, 5.2, 9, ppf(x = 9) - 0.6, col = "black", lty = 1, lwd = 2, arr.type = "triangle", arr.lwd = 0.5)


#Label the indifference curves
text(17, 6.6, expression(u[1]^A), cex = labelsize)
text(17, 7.8, expression(u[2]^A), cex = labelsize)
text(17, 9.6, expression(u[3]^A), cex = labelsize)
