...
```{r import Ancona}
AnconaBase <- read.csv("indmarketdemand/ancona.csv")
```
...
```{r}
pdf(file = "ancona_plot.pdf", width = 5, height = 4)
plot1 <- AnconaAve %>% 
  ggplot(aes(x = sumquantity, y = meanprice)) +
  geom_point() +
  stat_smooth(method = "lm", formula = y ~ x, size = 1, se = FALSE, color = "#0868ac") + 
  ylab("Daily average price per kilogram (euros)") +
  xlab("Daily quantity (kilograms)") +
  theme_bw() +
...
```{r}
coef(lm(sumquantity ~ meanprice, data = AnconaAve))
```
...
```{r}
pdf(file = "ancona_plot2.pdf", width = 5, height = 4)
AnconaAve %>% 
  ggplot(aes(x = sumquantity, y = meanprice)) +
  geom_point() +
  geom_abline(intercept = 9.2, slope = -0.006, color = "#0868ac", cex = 0.9)  +
  ylab(expression(paste("Daily average price per kilogram (Euros), ", p))) +
  ...
  annotate("text", x = 1000, y = 5.7, label = "Marginal Rate of") +
  annotate("text", x = 1000, y = 5.2, label = "Substitution, or") +
  annotate("text", x = 1000, y = 4.6, label = "Demand for x") +
  annotate("text", x = 53, y = 9.4, label = "bar(p)", parse = TRUE) +
  annotate("text", x = 1530, y = 3, label = "bar(x)", parse = TRUE) +
...
```{r}
pdf(file = "ancona_plot_checkslope.pdf", width = 5, height = 4)
AnconaAve %>% 
  ggplot(aes(x = sumquantity, y = meanprice)) +
  geom_point() +
  geom_abline(intercept = 16.8, slope = -0.033, col = "blue", cex = 0.9) +
