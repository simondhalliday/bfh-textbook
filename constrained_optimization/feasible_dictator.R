...
#Axis labels
mtext(expression(paste("C's payoff (dollars), ", pi^C)), side = 1, line = 2.5, cex = axislabelsize)
text(-1, 0.5*ylims[2], expression(paste("D's payoff (dollars), ", pi^D)), xpd = TRUE, cex = axislabelsize, srt = 90) 
...
text(10.5, 0.4, expression(u[1]^C), cex = labelsize)
text(10.5, 1.5, expression(u[2]^C), cex = labelsize)
text(10.5, 2.8, expression(u[3]^C), cex = labelsize)

#Annotate max u point on feasible frontier
text(7.2, ppf(7) + 0.2, expression(paste(i*minute)), cex = labelsize)
