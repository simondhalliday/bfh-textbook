...
library(extrafont)
library(pBrackets)
pdf(file = "employment/employment_rents_toplabels.pdf", width = 8, height = 6)
...
ylabels <- c(NA, expression(paste(B)), NA, expression(paste(w^N)), NA)
ticksx <- c(xlims[1], 0.06, 0.5, xlims[2])
xlabels <- c(NA, expression("Week 2"), expression("Week 28"), NA)
...
xpoly1 <- c(0.06, 0.5, 0.5, 0.06, 0.06)
ypoly1 <- c(0, 0, 0.2, 0.2, 0)
polygon(x = xpoly1, y = ypoly1, col=COLA[1], density=NULL, border = NA)

xpoly2 <- c(0.06, 0.5, 0.5, 0.06, 0.06)
ypoly2 <- c(0.2, 0.2, 0.4, 0.4, 0.2)
polygon(x = xpoly2, y = ypoly2, col="gray", density=NULL, border = NA)

xpoly3 <- c(0.06, 0.5, 0.5, 0.06, 0.06)
ypoly3 <- c(0.4, 0.4, 0.8, 0.8, 0.4)
polygon(x = xpoly3, y = ypoly3, col=COLB[1], density=NULL, border = NA)

#Label the feasible frontier
text(0.28, 0.3, expression("Disutility of effort if employed"), cex = labelsize)
text(0.28, 0.1, expression(paste("Unemployment benefit, ", s*B)), cex = labelsize)
text(0.28, 0.6, expression("Total employment"), cex = labelsize)
text(0.28, 0.55, expression("rent if employed"), cex = labelsize)

#Braces for labels
brackets(x1 = 0.51, y1 = 0.39, x2 = 0.51, y2 = 0.21,  ticks = 0.5, curvature = 0.5, type = 1, 
         col = "black", lwd = 2, lty = 1, xpd = FALSE)

text(0.61, 0.35, expression("Disutility"), cex = labelsize,xpd = TRUE)
text(0.61, 0.30, expression("of effort"), cex = labelsize,xpd = TRUE)
text(0.61, 0.25, expression("per week"), cex = labelsize,xpd = TRUE)
...
text(0.61, 0.125, expression("Benefits"), cex = labelsize,xpd = TRUE)
text(0.61, 0.075, expression("per week, B"), cex = labelsize,xpd = TRUE)
#text(0.62, 0.04, expression("of time"), cex = labelsize,xpd = TRUE)

#Axis labels
text(0.6, -0.2 , expression(paste("Time in weeks")), xpd = TRUE, cex = axislabelsize) 
text(-0.08, 0.8*(ylims[2]), expression(paste("Wage, ", w)), xpd = TRUE, cex = axislabelsize, srt = 90) 

#Spell of unemployment
brackets(x1 = 0.49, y1 = -0.08, x2 = 0.07, y2 = -0.08,  
         ticks = 0.5, curvature = 0.5, type = 1, 
         col = "black", lwd = 2, lty = 1, xpd = TRUE)

text(0.28, -0.15, expression(paste("Spell of unemployment, ", s == 26, " weeks")), cex = labelsize, xpd = TRUE)
...
Arrows(0, 0.78, 0.05, 0.78, lty = 1, col = COLA[4] , arr.type = "triangle", arr.lwd = segmentlinewidth)
Arrows(0.06, 0.78, 0.06, 0.22, lty = 1, col = COLA[4] , arr.type = "triangle", arr.lwd = segmentlinewidth)

Arrows(0.06, 0.2, 0.25, 0.2, lty = 1, col = COLA[4] , arr.type = "triangle", arr.lwd = segmentlinewidth)
...
text(0.06, 0.99, expression(paste("Employee")), cex = labelsize, xpd = TRUE)
text(0.06, 0.94, expression(paste("terminated")), cex = labelsize, xpd = TRUE)
Arrows(0.06, 0.91, 0.06, 0.83, lty = 1, col = "black", arr.type = "triangle", arr.lwd = segmentlinewidth, xpd = TRUE)

text(0.5, 0.99, expression(paste("Employee")), cex = labelsize, xpd = TRUE)
text(0.5, 0.94, expression(paste("re-hired")), cex = labelsize, xpd = TRUE)
Arrows(0.5, 0.91, 0.5, 0.83, lty = 1, col = "black", arr.type = "triangle", arr.lwd = segmentlinewidth, xpd = TRUE)
