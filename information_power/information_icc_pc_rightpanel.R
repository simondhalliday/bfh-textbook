require(ggplot2)
require(shape)
pdf(file = "information_power/information_icc_pc_rightpanel.pdf", width = 9, height = 7)
...
xlabels <- c(0, expression(paste(underline("u"))), expression(paste(p^C == 2*underline("u"))), expression(p^N == paste(4*underline("u"))), 40)
...
text(34.5, 0.55, expression(paste("Participation constraint")), cex = annotatesize, xpd = TRUE)
text(34.5, 0.51, expression(paste("(willingness to sell)")), cex = annotatesize, xpd = TRUE)
...
text(3.3, 0.048, expression(paste(v[0] == u[0])), cex = annotatesize - 0.05, xpd = TRUE)
text(3.6, 0.02, expression(phantom() == paste(0)), cex = annotatesize - 0.05, xpd = TRUE)
