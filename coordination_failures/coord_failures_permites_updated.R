require(shape)
pdf(file = "coordination_failures/coord_failures_permits_updated.pdf", width = 9, height = 7)
...
uA <- function(ea, eb, alpha = 20, beta = 1/20, gamma = 2) {
  alpha*ea*(1 - beta*(eb+ea))- gamma*ea
}

uB <- function(ea, eb, alpha = 20, beta = 1/20, gamma = 2) {
  alpha*(1 - beta*(eb + ea) )*eb - gamma*(eb)
}

brfB <- function(ea, alpha = 20, beta = 1/20, gamma = 2) {
  (alpha - beta*ea - gamma)/(2*beta)
}

brfA <- function(ea, alpha = 20, beta = 1/20, gamma = 2) {
  (alpha - 2*beta*ea - gamma)/beta
}

brfPE <- function(ea, alpha = 20, beta = 1/8) {
  alpha*(1 - beta*ea)
}

hAmonop <- function(alpha, beta = 1/20, gamma = 2){
  (alpha - gamma)/(2*beta)
}


hANE <- function(alpha, beta = 1/20, gamma = 2){
  (alpha - gamma)/(3*beta)
}

hApEff <- function(alpha, beta = 1/20){
  
}

indiffeA <- function(ea, alpha = 20, beta = 1/20, uA = 46.08) {
  1/beta - (0.5*ea)/(alpha*beta) - uA/(alpha*beta*ea)
}

indiffB <- function(eb, alpha = 20, beta = 1/20, uB = 46.08) {
  (alpha*eb - uB - 0.5*eb^2)/(alpha*beta*eb)
}

indiffB2 <- function(ea, alpha = 20, beta = 1/20, uB = 46.08) {
  sqrt(alpha^2*(beta*ea - 1)^2 - 2*uB) - alpha*beta*ea + alpha
}

indiffB3 <- function(ea, alpha = 20, beta = 1/20, uB = 46.08) {
  -sqrt(alpha^2*(beta*ea - 1)^2 - 2*uB) - alpha*beta*ea + alpha
}
...
# indiffAlowB <- function(eb, alpha = 20, beta = 1/20, uA = 46.08) {
#   0.5*(-sqrt((2*alpha*beta*eb - 2*alpha)^2 - 8*uA)) - 2*alpha*beta*eb + 2*alpha
# }
# 
# indiffAlow <- function(ea, alpha = 20, beta = 1/20, uA = 46.08) {
#   0.5*(-sqrt((2*alpha*beta*ea - 2*alpha)^2 - 8*uA)) - 2*alpha*beta*ea + 2*alpha
# }
# 
# indiffAgain <- function(ea, alpha = 20, beta = 1/20, uA = 46.08) {
...
ypoly1 <- indiffA(xpoly1, uA = 46.08, alpha = 20, beta = 1/20)
ypoly2 <- indiffB3(xpoly1, uB = 46.08, alpha = 20, beta = 1/20)
polygon(x = c(xpoly1, rev(xpoly1)), y = c(ypoly1, rev(ypoly2)), col=COL[4], density=NULL, border = NA)



lines(xx1, brfA(xx1, alpha = 20, beta = 1/20), col = COLA[4], lwd = graphlinewidth)
lines(xx1, brfB(xx1, alpha = 20, beta = 1/20), col = COLB[4], lwd = graphlinewidth)
#lines(xx1, indiffA(xx1, alpha = 20, beta = 1/20, uA = 46.08), col = COL[2], lwd = graphlinewidth)
#lines(xx1, indiffB(xx1, alpha = 20, beta = 1/20, uB = 42), col = COL[2], lwd = graphlinewidth)
#lines(xx1, indiffB2(xx1, alpha = 20, beta = 1/20, uB = 46.08), col = COL[2], lwd = graphlinewidth)
#lines(xx1, indiffB3(xx1, alpha = 20, beta = 1/20, uB = 46.08), col = COL[2], lwd = graphlinewidth)
#lines(xx1, indiffAlow(xx1, uA = 42, alpha = 20, beta = 1/20), col = COL[2], lwd = graphlinewidth)
#lines(xx1, indiffAgain(xx1, uA = 42, alpha = 20, beta = 1/20), col = COL[2], lwd = graphlinewidth)
