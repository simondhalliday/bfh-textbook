require(shape)
pdf(file = "employment/employment_brf_pc_leftpanelv2.pdf", width = 9, height = 7)
...
xx8 <- seq(14.5, 22, length.out = npts2)
...
ticksy <- c(0, 0.5, 1)
ylabels <- c(0, expression(paste(e^N == frac(1,2))), 1)
ticksx <- c(0, 10, 20, 40)
xlabels <- c(0, expression(paste(w == 2*underline("u"))), expression(paste(w^N)) , 40)
...
text(30, 0.5, expression(paste("Better for employee")), cex = annotatesize)
...
#segments(18, 0.2, 18, 0.65, lty = 2, col = grays[20], lwd = segmentlinewidth)
