...
library(shape)
pdf(file = "constrained_optimization/feasible_frontier_indiff_section.pdf", width = 8, height = 8)
...
ppf <- function(x, slope = 1/20, bary = 20) {
  bary - slope *x^2
}
...
indiffA <- function(x, ubar = uFn(8,3) - 1, alpha = 0.5) {
...
a <- c(uFn((400/3)^0.5, ((400/3)^0.5)^2/10) - 1, uFn((400/3)^0.5, ((400/3)^0.5)^2/10), uFn((400/3)^0.5, ((400/3)^0.5)^2/10)+ 1.4) #alpha = 0.4
...
#Axis labels
mtext(expression(paste("Fish (kilograms), ", x)), side = 1, line = 2.5, cex = axislabelsize)
text(-1.6, 0.5*ylims[2], expression(paste("Coconuts (kilograms), ", y)), xpd = TRUE, cex = axislabelsize, srt = 90) 

#Add mrs = mrt at i
points((400/3)^0.5, ppf(x = (400/3)^0.5), pch = 16, col = "black", cex = 1.5)
text((400/3)^0.5, 5, expression(paste(mrs(x,y) == mrt(x,y))), cex = labelsize)
text((400/3)^0.5 + 0.3, 3.6, expression(paste(frac(y,x) == frac(1,10)*x)), cex = labelsize)
Arrows((400/3)^0.5, 5.2, (400/3)^0.5, 12.7, col = "black", lty = 1, lwd = 2, arr.type = "triangle", arr.lwd = 0.5)


#Label the indifference curves
text(19, 6.2, expression(u[1]^A), cex = labelsize)
text(19, 7.5, expression(u[2]^A), cex = labelsize)
text(19, 9.4, expression(u[3]^A), cex = labelsize)
