...
library(shape)
pdf(file = "what_can_markets_do/curfew_indifference_b2.pdf", width = 10, height = 8)
...
ticksx <- c(3, 6, 9)
xlabels <- c(expression(paste(T^A == 9*p*m)), expression(paste(12*a*m)),  expression(paste(T^B  == 3*a*m)))
...
text(xlims[1] - 0.5, 0.5*ylims[2], expression(paste("Money given or payment to A, ", y)), xpd = TRUE, cex = axislabelsize, srt = 90) 
...
text(7.5, 8.5, expression(paste("Better for B")), 
...
text(4.6, 9, expression(paste( u[1]^B )), xpd = TRUE, cex = labelsize)
text(4.2, 9, expression(paste( u[2]^B )), xpd = TRUE, cex = labelsize)
text(3.9, 9, expression(paste( u[3]^B )), xpd = TRUE, cex = labelsize)
