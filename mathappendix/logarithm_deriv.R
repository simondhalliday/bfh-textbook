...
library(shape)
pdf(file = "mathappendix/logarithm_deriv.pdf", width = 8, height = 6)
...
ticksx <- c(xlims[1], 2, 4, 6, xlims[2])
xlabels <- c(NA, 2, 4, 6, expression(x))
ticksy <- c(ylims[1], 2, 4, 6, ylims[2])
ylabels <- c(0, 2, 4, 6, expression(y))
...
# lines
lines(xx1, 1/(xx1), col = COLB[4], lwd = graphlinewidth)
