require(shape)
pdf(file = "competitionmarkets/ds_core_excess_s.pdf", width = 9, height = 7)
...
ylabels <- c(NA,  expression(paste(p[d])), expression(paste(p,"*")), expression(paste(p[H])), NA, NA)
ticksx <- c(0, 54, 75, 110, 120, xlims[2])
xlabels <- c(NA, expression(paste(X[{DH}])), expression(paste(X,"*")), expression(paste(X[{SH}])), NA, NA)
