...
library(pBrackets)
require(plotrix)
pdf(file = "employment/contractual_completeness_degree.pdf", width = 9, height = 7)
...
ticksy <- c(0, 1, 4.5, 5)
ylabels <- c(0, expression(paste(B + underline(u))), NA, NA)
#ticksx <-seq(0, 1, 0.1)
ticksx <- c(0, 0.22, 0.5, 1)
#xlabels <-seq(0, 1, 0.1)
xlabels <- c(0, NA, expression(paste(t == 0.5)), 1)
...
segments(0.5, 0, 0.5, 1,lty = 2, lwd = 1.5, col = "darkgray", xpd = TRUE)
...
#x
text(0.22, -0.25, expression(paste(underline(t))), cex = labelsize, xpd = TRUE)
...
# #Arrows
Arrows(0.5, 1.65, 0.5, 1.88, col = "black", lty = 1, lwd = 2, arr.type = "triangle")
Arrows(0.5, 1.42, 0.5, 1.1, col = "black", lty = 1, lwd = 2, arr.type = "triangle")
Arrows(0.5, 3.9, 0.5, 4.4, col = "black", lty = 1, lwd = 2, arr.type = "triangle")
Arrows(0.5, 3.65, 0.5, 2.15, col = "black", lty = 1, lwd = 2, arr.type = "triangle")
...
# text(0.54, 4, expression(paste(j == 0.8)), cex = labelsize, xpd = TRUE)
