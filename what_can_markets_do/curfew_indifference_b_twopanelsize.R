...
library(shape)
pdf(file = "what_can_markets_do/curfew_indifference_b_twopanelsize.pdf", width = 7, height = 5)
...
text(xlims[1] - 1.23, 0, expression(paste("Difference in income, ", y^B)), xpd = TRUE, cex = axislabelsize, srt = 90) 
...
text(2.5, 9, expression(paste( u[3]^B == -18 )), xpd = TRUE, cex = labelsize)
text(5.7, 9, expression(paste( u[5]^B == 0)), xpd = TRUE, cex = labelsize)
text(5.7, 17.2, expression(paste( u[7]^B == 9)), xpd = TRUE, cex = labelsize)

#Tangent segments
segments(8, 9, 10, 9, lty = 2, col = grays[22], lwd = segmentlinewidth, xpd = TRUE)
segments(8, -18, 10, -18, lty = 2, col = grays[22], lwd = segmentlinewidth, xpd = TRUE)
...
# text(7.75, -12.8, expression(paste("Later")), 
#      xpd = TRUE, cex = axislabelsize )

#Better for B arrow and label
text(8, 13.25, expression(paste("Better for B")), 
...
text(4.3, -13, expression(paste("slope ", phantom()==-mrs )), 
     xpd = TRUE, cex = annotatesize)
text(5.35, -16, expression(paste(phantom()==-2*beta(T - T^B))), 
     xpd = TRUE, cex = annotatesize)

Arrows(4.35, -12, 4.35, -8.5,
       col = "black", lty = 1, lwd = 2,
       arr.type = "triangle",
       arr.lwd = 0.5, code = 2)
