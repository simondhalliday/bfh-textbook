...
text(10.2, 19.5, expression(paste(mrs^{A}*(list(h^A, h^B)) == frac(u[h^A], u[h^B]), phantom() == -frac((alpha - beta*h^B) - h^A*(1 + 2*beta), beta*h^A), phantom() == 0, " at maximum of ", u^A)), xpd = TRUE)
