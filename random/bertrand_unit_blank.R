...
pdf(file = "random/bertrand_unit_blank.pdf", width = 6, height = 3.5)
...
text(4.5, 1.2, expression(paste(d(x,Pixel)==1-x)), 
...
brackets(x1 = 3, y1 = 0.83, x2 = 0, y2 = 0.83,  
         ticks = 0.5, curvature = 0.5, type = 1, 
         col = "black", lwd = 2, lty = 1, xpd = TRUE)
text(1.5, 0.67, expression(paste(d(x,iPhone)==x)), 
     xpd = TRUE, srt = 0, cex = annotatesize)
...
#      xpd = TRUE, srt = 0, cex = annotatesize)
# 
# text(4.32, 0.5, labels=emoji("neutral_face"), family="OpenSansEmoji", cex=1, xpd = TRUE)
