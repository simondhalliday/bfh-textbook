...
library(extrafont)
library(pBrackets)
pdf(file = "what_can_markets_do/edgeworthbox_simulation.pdf", width = 9, height = 7)
...
uA <- function(x, y, rmax = 10, xmax = 50) {
  y + rmax*x - (1/2)*(rmax/xmax)*x^2
}

indiffA <- function(x, utility = uA(x = 5, y = 150), rmax = 10, xmax = 50) {
  utility - rmax*x + (1/2)*(rmax/xmax)*x^2
}

mrsA <- function(x, rmax = 10, xmax = 50) {
  rmax - (rmax/xmax)*x
}

uB <- function(x, y, rmax = 100, xmax = 10) {
  (300 - y) + rmax*(10 - x) - (1/2)*(rmax/xmax)*(10 - x)^2
}

indiffB <- function(x, utility = uB(x = 5, y = 150), rmax = 100, xmax = 10) {
  utility + rmax*(x) - (1/2)*(rmax/xmax)*(x)^2
}

WalrasP <- function(x, intercept = 199, pw = mrsA(x = 0.98)) {
  intercept - pw*x
}
...
a <- c(100, uA(x = 5, y = 150), 280, 300)
b <- c(400, uB(x = 5, y = 150), 608)
...
ticksy <- seq(from = 0, to = 300, by = 30)
ylabels <- seq(from = 0, to = 300, by = 30)
...
#Add arrows:
arrows(-0.8, 154, -0.8, 210, xpd = TRUE, length = 0.1, angle = 40, lwd = 3)
arrows(6.2, -27, 9, -27, xpd = TRUE, length = 0.1, angle = 40, lwd = 3)
...
# 
segments(1, 0, 1, ylims[2], col = COL[2] , lwd = segmentlinewidth, lty = 2)
# 
# 
# #Label the PEC
text(3.4, 44, expression("Pareto Efficient Curve"))
Arrows(2.2, 44, 1.2, 44, col = "black", lty = 1, lwd = 2, arr.type = "triangle", arr.lwd = 0.5)
# 
...
text(9.35, 23, expression(u[1]^A))
text(9.35, 80, expression(u[2]^A))
text(9.35, 163, expression(u[3]^A))
# 
# #Label the indifference curves for the HG, Betty
text(5, 10, expression(u[1]^B))
text(6.5, 10, expression(u[2]^B))
text(7.6, 10, expression(u[3]^B))
...
# 
# #Initial Allocations
points(x = 5, y = 150, pch = 16, col = "black", cex = 1.5)
text(5, 140, expression(paste(e)))

points(x = 0.98, y = mrsA(x = 0.98)*(5) + 150 - mrsA(x = 0.98)*(0.98), pch = 16, col = "black", cex = 1.5)
text(1.1, 195, expression(paste(n)))

#Pareto-improving lens

# xpoly1 <- seq(from = 0, to = 5, length.out = 500)
# ypoly1 <- indiffB(xpoly1)
# ypoly2 <- WalrasP(xpoly1)
# polygon(x = c(xpoly1, rev(xpoly1)), y = c(ypoly1, rev(ypoly2)), col = COL[4], density = NULL, border = NA)
