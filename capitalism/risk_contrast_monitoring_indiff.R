require(shape)
library(pBrackets)
pdf(file = "capitalism/risk_contrast_monitoring_indiff.pdf", width = 9, height = 7)
...
#segments(0, riskreturn(int1 = 15, g = 12), xlims[2], riskreturn(int1 = 15, g = 12), lty = 1, col = COLB[4], lwd = graphlinewidth)
...
segments(0, 11, xlims[2], 11, lty = 1, col = COLB[4], lwd = graphlinewidth)

segments(0, 6, xlims[2], 6, lty = 1, col = COLB[4], lwd = graphlinewidth)
#segments(0, 11, xlims[2], 11, lty = 1, col = COLB[4], lwd = graphlinewidth)

points(12, riskreturn(int1 = 15, g = 12), pch = 16, col = "black", cex = 1.5)
text(12, riskreturn(int1 = 15, g = 12) + 0.6, expression(paste(g) ), cex = labelsize)

points(12, riskreturn(int1 = 11, g = 12), pch = 16, col = "black", cex = 1.5)
text(12, riskreturn(int1 = 11, g = 12) + 0.6, expression(paste(f) ), cex = labelsize)
...
text(9.8, riskreturn(int1 = 11, g = 14)-2, expression(paste(w^c==hat(y)*(Delta)-rho%.%k-m) ), cex = labelsize)
...
#Label value functions
text(2, 10.5, expression(u[2]), cex = labelsize)
text(2, 5.5, expression(u[1]), cex = labelsize)

text(4.2, 15, expression(paste("Better for")), cex = labelsize)
text(4.2, 14, expression(paste("owner")), cex = labelsize)
Arrows(2.75, 13, 2.75, 16,
